...
using Oceananigans.Grids: znode, Face, topology, Flat
...
function ReferenceState(grid, constants=ThermodynamicConstants(eltype(grid));
    ...
    pˢᵗ = convert(eltype(grid), standard_pressure)
    return materialize_reference_state(grid, constants, base_pressure, potential_temperature, pˢᵗ,
                                       discrete_hydrostatic_balance,
                                       vapor_mass_fraction, liquid_mass_fraction, ice_mass_fraction)
end

# Homogeneous reference state: a constant `base_pressure` and a constant or `z`-function
# `potential_temperature` give one profile shared by every column. Array-valued parameters dispatch
# instead to the per-column methods below (`materialize_reference_state(..., ::AbstractArray, ...)`).
function materialize_reference_state(grid, constants, base_pressure::Number,
                                     potential_temperature::Union{Number, Function}, pˢᵗ,
                                     discrete_hydrostatic_balance, vapor_mass_fraction,
                                     liquid_mass_fraction, ice_mass_fraction)
...
#####
##### Per-column (heterogeneous) reference state
#####
#
# Array-valued `base_pressure` and/or `potential_temperature` give each column of an ensemble its
# own adiabatic profile. Dispatching on the parameter type (rather than an `isa` predicate) keeps the
# homogeneous and per-column paths as distinct methods.

materialize_reference_state(grid, constants, base_pressure::AbstractArray, potential_temperature, pˢᵗ, args...) =
    column_reference_state(grid, constants, base_pressure, potential_temperature, pˢᵗ, args...)

materialize_reference_state(grid, constants, base_pressure::Number, potential_temperature::AbstractArray, pˢᵗ, args...) =
    column_reference_state(grid, constants, base_pressure, potential_temperature, pˢᵗ, args...)
...
Construct a per-column `ReferenceState` on a column-ensemble grid, where `base_pressure` and/or
...
scalar `base_pressure`/`potential_temperature` metadata store the first column's surface values.
"""
function column_reference_state(grid, constants, base_pressure, potential_temperature, pˢᵗ,
                                discrete_hydrostatic_balance,
                                vapor_mass_fraction, liquid_mass_fraction, ice_mass_fraction)
    ...
    is_column_ensemble(grid) ||
        throw(ArgumentError("Array-valued `base_pressure`/`potential_temperature` require a " *
                            "column-ensemble grid (Flat horizontal topology with size > 1)."))

    validate_column_parameter(base_pressure, Nx, Ny, "base_pressure")
    ...
    p₀ = column_parameter(base_pressure, FT, arch)
    ...
    # Each column carries its own bottom-face anchor, reduced from that column's own datum.
    pˢ = Field{Center, Center, Nothing}(grid)
    ρˢ = Field{Center, Center, Nothing}(grid)
    Tˢ = Field{Center, Center, Nothing}(grid)

    launch!(arch, grid, :xy, _compute_adiabatic_reference_columns!,
            ρᵣ, pᵣ, Tᵣ, pˢ, ρˢ, Tˢ, grid, Nz, p₀, θ₀, pˢᵗ, constants)
    fill_halo_regions!(ρᵣ)
    fill_halo_regions!(pᵣ)
    fill_halo_regions!(Tᵣ)
    fill_halo_regions!(pˢ)
    fill_halo_regions!(ρˢ)
    fill_halo_regions!(Tˢ)
    ...
    return ReferenceState(p₀₁₁, pˢ, ρˢ, Tˢ, θ₀₁₁, pˢᵗ, pᵣ, ρᵣ, Tᵣ, Z, Z, Z)
end

@kernel function _compute_adiabatic_reference_columns!(ρᵣ, pᵣ, Tᵣ, pˢ, ρˢ, Tˢ, grid, Nz, p₀, θ₀, pˢᵗ, constants)
    ...
    @inbounds begin
        pˢ[i, j, 1] = adiabatic_hydrostatic_pressure(zˢ, p₀ᵢⱼ, θ₀ᵢⱼ, pˢᵗ, constants)
        ρˢ[i, j, 1] = adiabatic_hydrostatic_density(zˢ, p₀ᵢⱼ, θ₀ᵢⱼ, pˢᵗ, constants)
        Tˢ[i, j, 1] = hydrostatic_temperature(zˢ, p₀ᵢⱼ, θ₀ᵢⱼ, pˢᵗ, constants)
    end
