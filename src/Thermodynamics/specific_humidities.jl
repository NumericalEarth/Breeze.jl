struct MassRatios{FT}
    vapor :: FT
    liquid :: FT
    ice :: FT
end

@inline total_specific_humidity(q::MassRatios) = q.vapor + q.liquid + q.ice
@inline dry_air_mass_fraction(q::MassRatios) = 1 - total_specific_humidity(q)

"""
    mixture_gas_constant(q::MassRatios, thermo)
...
"""
@inline function mixture_gas_constant(q::MassRatios, thermo::TC)
...
liquid condensate.
"""
@inline function mixture_heat_capacity(q::MassRatios, thermo::TC)
    qᵈ = dry_air_mass_fraction(q)
    qᵛ = q.vapor
