...
@kernel function _update_filtered_Δθᵥ!(Δθ̂ᵥ, coef, T₀, grid, clock, fields, ϵ)
    i, j = @index(Global, NTuple)
    T₀ᵢⱼ = wall_value(i, j, grid, Bottom(), T₀, clock)
    p₀ = wall_air_pressure(i, j, 1, grid, Bottom(), nothing, fields, coef.thermodynamic_constants)
    Δθᵥⁿ = surface_layer_Δθᵥ(i, j, 1, grid, coef, T₀ᵢⱼ, fields, p₀)
...
@kernel function _initialize_filtered_Δθᵥ!(Δθ̂ᵥ, coef, T₀, grid, clock, fields)
    i, j = @index(Global, NTuple)
    T₀ᵢⱼ = wall_value(i, j, grid, Bottom(), T₀, clock)
    p₀ = wall_air_pressure(i, j, 1, grid, Bottom(), nothing, fields, coef.thermodynamic_constants)
    @inbounds Δθ̂ᵥ[i, j, 1] = surface_layer_Δθᵥ(i, j, 1, grid, coef, T₀ᵢⱼ, fields, p₀)
end
...
"""
    initialize_Δθᵥ!(fv::FilteredSurfaceVelocities, coef, T₀, grid, clock, fields)
...
"""
function initialize_Δθᵥ!(fv::FilteredSurfaceVelocities, coef, T₀, grid, clock, fields)
    arch = architecture(grid)
    kp = filtered_kernel_parameters(grid)
    launch!(arch, grid, kp, _initialize_filtered_Δθᵥ!, fv.Δθᵥ, coef, T₀, grid, clock, fields)
    return nothing
end

"""
    update_Δθᵥ!(fv::FilteredSurfaceVelocities, coef, T₀, grid, clock, Δt, fields)

Apply the exponential filter to the surface-layer virtual potential temperature difference,
with the surface temperature `T₀` evaluated at `clock.time`. `fields` carries the same
requirement as [`initialize_Δθᵥ!`](@ref).
"""
function update_Δθᵥ!(fv::FilteredSurfaceVelocities, coef, T₀, grid, clock, Δt, fields)
    arch = architecture(grid)
    kp = filtered_kernel_parameters(grid)
    ϵ = Δt / fv.filter_timescale
    launch!(arch, grid, kp, _update_filtered_Δθᵥ!, fv.Δθᵥ, coef, T₀, grid, clock, fields, ϵ)
    return nothing
end
