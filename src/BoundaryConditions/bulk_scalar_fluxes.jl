...
struct BulkSensibleHeatFluxFunction{C, G, T, SP, TC, F, FV, FS}
    coefficient :: C
    gustiness :: G
...
@inline bulk_sensible_heat_difference(i, j, grid, ::Nothing, bf, T₀, fields, p₀) =
    bulk_sensible_heat_difference(i, j, grid, PotentialTemperatureFlux(), bf, T₀, fields, p₀, nothing)
@inline bulk_sensible_heat_difference(i, j, grid, ::Nothing, bf, T₀, fields, p₀, fs) =
    bulk_sensible_heat_difference(i, j, grid, PotentialTemperatureFlux(), bf, T₀, fields, p₀, fs)

# No filtered scalar: read from 3D fields (current behavior)
@inline function bulk_sensible_heat_difference(i, j, grid, ::PotentialTemperatureFlux, bf, T₀, fields, p₀, ::Nothing)
...
@inline function bulk_sensible_heat_difference(i, j, grid, ::PotentialTemperatureFlux, bf, T₀, fields, p₀, fs::FilteredSurfaceScalar)
...
@inline function OceananigansBC.getbc(bf::BulkSensibleHeatFluxFunction, i::Integer, j::Integer,
    ...
    constants = bf.thermodynamic_constants
    p₀ = surface_air_pressure(i, j, grid, fields, constants)
    ρ₀ = surface_density(p₀, T₀, constants)

    Cᵀ = bulk_coefficient(i, j, grid, bf.coefficient, fields, T₀, bf.filtered_velocities, p₀)

    Δϕ = bulk_sensible_heat_difference(i, j, grid, bf.formulation, bf, T₀, fields, p₀, bf.filtered_scalar)
    return - ρ₀ * Cᵀ * Ũ * Δϕ
end
...
struct BulkVaporFluxFunction{C, G, T, TC, S, FV, FS}
    coefficient :: C
    gustiness :: G
    surface_temperature :: T
    thermodynamic_constants :: TC
...
function BulkVaporFluxFunction(; coefficient, gustiness=0, surface_temperature, filtered_velocities=nothing)
    return BulkVaporFluxFunction(coefficient, gustiness, surface_temperature,
                                  nothing, nothing, filtered_velocities, nothing)
end
...
@inline function OceananigansBC.getbc(bf::BulkVaporFluxFunction, i::Integer, j::Integer,
    ...
    Cᵛ = bulk_coefficient(i, j, grid, bf.coefficient, fields, T₀, bf.filtered_velocities, p₀)
