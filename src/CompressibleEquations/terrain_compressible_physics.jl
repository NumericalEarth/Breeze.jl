...
# args...)` to a Center, used to gate the terrain slope corrections on a specified side
# (SubstepBoundaryUpdate). It is the Bool-weighted average `Σ aₚ qₚ / max(Σ aₚ, 1)`
...
@inline function x_masked_face_value(i, j, k, grid, specified_sides, q::Q, args...) where Q
    x_specified, _ = specified_zone_faces(i, j, grid, specified_sides)
    return ifelse(x_specified, zero(grid), q(i, j, k, grid, args...))
end

@inline function y_masked_face_value(i, j, k, grid, specified_sides, q::Q, args...) where Q
    _, y_specified = specified_zone_faces(i, j, grid, specified_sides)
    return ifelse(y_specified, zero(grid), q(i, j, k, grid, args...))
end

@inline function x_unspecified_weight(i, j, k, grid, specified_sides)
    x_specified, _ = specified_zone_faces(i, j, grid, specified_sides)
    return ifelse(x_specified, zero(grid), one(eltype(grid)))
end

@inline function y_unspecified_weight(i, j, k, grid, specified_sides)
    _, y_specified = specified_zone_faces(i, j, grid, specified_sides)
    return ifelse(y_specified, zero(grid), one(eltype(grid)))
end

@inline function one_sided_ℑxᶜᵃᵃ(i, j, k, grid, specified_sides, q::Q, args...) where Q
    num = ℑxᶜᵃᵃ(i, j, k, grid, x_masked_face_value, specified_sides, q, args...)
    den = ℑxᶜᵃᵃ(i, j, k, grid, x_unspecified_weight, specified_sides)
    return 2num / max(2den, one(den))
end

@inline function one_sided_ℑyᵃᶜᵃ(i, j, k, grid, specified_sides, q::Q, args...) where Q
    num = ℑyᵃᶜᵃ(i, j, k, grid, y_masked_face_value, specified_sides, q, args...)
    den = ℑyᵃᶜᵃ(i, j, k, grid, y_unspecified_weight, specified_sides)
    return 2num / max(2den, one(den))
end

# Full terrain horizontal PGF projected onto the vertical (contravariant) slow tendency.
# `specified_sides` gates it exactly as the linearized correction below: on a specified side the
...
# (a leak channel: SubstepBoundaryUpdate). The `::Nothing` method (no side specified —
# every unspecified terrain model) is the original plain interpolation, BIT-IDENTICAL.
...
@inline function terrain_horizontal_pressure_gradient_correction(i, j, k, grid, dynamics, specified_sides::OpenSides)
    slope_x = terrain_slope_x_ccf(i, j, k, grid)
    slope_y = terrain_slope_y_ccf(i, j, k, grid)
    ∂x_p_ccf = ℑzᵃᵃᶠ(i, j, k, grid, one_sided_ℑxᶜᵃᵃ, specified_sides, AtmosphereModels.x_pressure_gradient, dynamics)
    ∂y_p_ccf = ℑzᵃᵃᶠ(i, j, k, grid, one_sided_ℑyᵃᶜᵃ, specified_sides, AtmosphereModels.y_pressure_gradient, dynamics)
    return slope_x * ∂x_p_ccf + slope_y * ∂y_p_ccf
end

# `specified_sides` gates the horizontal linearized-PGF correction so the terrain slope
# projection never reads a specified face — where the acoustic ∂p′ is switched
# off in `_explicit_horizontal_step!`, the slope-projected correction must be switched off
# in lockstep, else the two are out of phase and specified-cell perturbation pressure leaks
# into interior columns through the non-column-local stencil.
#
# The `::Nothing` method (no side specified — every flat model and every unspecified terrain
...
# Specified: interpolate ∇ˣp′/∇ʸp′ to (Center, Center) with the one-sided, specified-face-
...
@inline function terrain_horizontal_linearized_pressure_gradient_correction(i, j, k, grid,
                                                                            dynamics, ρθ′, Πᴸ, γRᵐᴸ, specified_sides::OpenSides)
    slope_x = terrain_slope_x_ccf(i, j, k, grid)
    slope_y = terrain_slope_y_ccf(i, j, k, grid)
    ∂x_p′_ccf = ℑzᵃᵃᶠ(i, j, k, grid, one_sided_ℑxᶜᵃᵃ, specified_sides, ∇ˣp′, dynamics, ρθ′, Πᴸ, γRᵐᴸ)
    ∂y_p′_ccf = ℑzᵃᵃᶠ(i, j, k, grid, one_sided_ℑyᵃᶜᵃ, specified_sides, ∇ʸp′, dynamics, ρθ′, Πᴸ, γRᵐᴸ)
    return slope_x * ∂x_p′_ccf + slope_y * ∂y_p′_ccf
end
...
@inline function ∇ᶻp′(i, j, k, grid,
                                                dynamics::TerrainCompressibleDynamics,
                                                ρθ′, Πᴸ, γRᵐᴸ, slope_correction, specified_sides)
...
# the specified boundary tendency ∂ₜ(ρu) replaces the interior Gⁿρu (which never drives a
# specified face). The `::Nothing` method (no side specified / no boundary field allocated)
# returns Gⁿρu unchanged, so the unspecified path is BIT-IDENTICAL. `oftype` keeps the
...
@inline specified_slow_x_momentum_tendency(i, j, k, grid, Gⁿρu, ::Nothing, specified_sides) =
    @inbounds Gⁿρu[i, j, k]

@inline function specified_slow_x_momentum_tendency(i, j, k, grid, Gⁿρu, ∂ₜρu, specified_sides)
    x_specified, _ = specified_zone_faces(i, j, grid, specified_sides)
...
@inline specified_slow_y_momentum_tendency(i, j, k, grid, Gⁿρv, ::Nothing, specified_sides) =
    @inbounds Gⁿρv[i, j, k]

@inline function specified_slow_y_momentum_tendency(i, j, k, grid, Gⁿρv, ∂ₜρv, specified_sides)
    _, y_specified = specified_zone_faces(i, j, grid, specified_sides)
...
function assemble_slow_vertical_momentum_tendency!(substepper::AcousticSubstepper,
    ...
    # SubstepBoundaryUpdate: `nothing` when no momentum BC carries the scheme, so
...
@kernel function _assemble_terrain_slow_vertical_momentum_tendency!(Gˢρw̃,
                                                                    ...
                                                                    grid, dynamics, g,
                                                                    vertical_pressure_tendency_factor,
                                                                    ∂ₜρu_boundary, ∂ₜρv_boundary, specified_sides)
    i, j, k = @index(Global, NTuple)

    slope_x = terrain_slope_x_ccf(i, j, k, grid)
    slope_y = terrain_slope_y_ccf(i, j, k, grid)
    # SubstepBoundaryUpdate: a specified x-/y-face takes its specified boundary
    # tendency in place of Gⁿρu/Gⁿρv (see the helpers above); unspecified builds are
    # bit-identical to the plain `ℑ`-of-Gⁿ interpolation.
    Gⁿρu_ccf = ℑzᵃᵃᶠ(i, j, k, grid, ℑxᶜᵃᵃ, specified_slow_x_momentum_tendency, Gⁿρu, ∂ₜρu_boundary, specified_sides)
    Gⁿρv_ccf = ℑzᵃᵃᶠ(i, j, k, grid, ℑyᵃᶜᵃ, specified_slow_y_momentum_tendency, Gⁿρv, ∂ₜρv_boundary, specified_sides)
    ...
    horizontal_pressure_gradient = terrain_horizontal_pressure_gradient_correction(i, j, k, grid, dynamics, specified_sides)
