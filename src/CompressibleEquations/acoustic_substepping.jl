...
"""
struct MaterializedHorizontalPressureIncrementDiagnostics{MP} <: AcousticSubstepperDiagnostics
    momentum_increment :: MP
    frozen_momentum_increment :: MP
    perturbation_momentum_increment :: MP
    horizontal_momentum_increment :: MP
    terrain_momentum_increment :: MP
    frozen_horizontal_momentum_increment :: MP
    frozen_terrain_momentum_increment :: MP
    perturbation_horizontal_momentum_increment :: MP
    perturbation_terrain_momentum_increment :: MP
    ungated_horizontal_momentum_increment :: MP
    ungated_terrain_momentum_increment :: MP
    slow_momentum_increment :: MP
    initial_momentum_perturbation :: MP
    final_momentum_perturbation :: MP
    exner_acceleration :: MP
    frozen_exner_acceleration :: MP
    perturbation_exner_acceleration :: MP
    horizontal_exner_acceleration :: MP
    terrain_exner_acceleration :: MP
    frozen_horizontal_exner_acceleration :: MP
    frozen_terrain_exner_acceleration :: MP
    perturbation_horizontal_exner_acceleration :: MP
    perturbation_terrain_exner_acceleration :: MP
    ungated_horizontal_exner_acceleration :: MP
    ungated_terrain_exner_acceleration :: MP
    nonlinear_pressure_acceleration :: MP
    nonlinear_horizontal_pressure_acceleration :: MP
    nonlinear_terrain_pressure_acceleration :: MP
    predictor_pressure_acceleration :: MP
    predictor_horizontal_pressure_acceleration :: MP
    predictor_terrain_pressure_acceleration :: MP
    recovered_pressure_acceleration :: MP
    recovered_horizontal_pressure_acceleration :: MP
    recovered_terrain_pressure_acceleration :: MP
end

struct BottomPressureModalSample
    time :: Float64
    outer_iteration :: Int
    stage_fraction :: Float64
    substep :: Int
    substeps :: Int
    state :: Symbol
    k :: Int
    mode :: Int
    theta_amplitude :: Float64
    theta_phase_degrees :: Float64
    pressure_amplitude :: Float64
    pressure_phase_degrees :: Float64
    ρu_amplitude :: Float64
    ρu_phase_degrees :: Float64
    ρw_amplitude :: Float64
    ρw_phase_degrees :: Float64
    u_amplitude :: Float64
    u_phase_degrees :: Float64
    w_tilde_amplitude :: Float64
    w_tilde_phase_degrees :: Float64
end

struct StepBTermModalSample
    time :: Float64
    outer_iteration :: Int
    stage_fraction :: Float64
    substep :: Int
...
mutable struct MaterializedBottomPressureModalDiagnostics{D, Θ, R, W, S} <: AcousticSubstepperDiagnostics
    pressure_diagnostics :: D
    theta_terms :: Θ
    density_predictor :: R
    vertical_terms :: W
    slow_vertical_terms :: S
    modes :: Vector{Int}
    levels :: Vector{Int}
    samples :: Vector{BottomPressureModalSample}
    step_b_samples :: Vector{StepBTermModalSample}
    rk_stage_samples :: Vector{RKStageModalSample}
end

materialize_acoustic_substepper_diagnostics(grid, ::NoAcousticSubstepperDiagnostics) =
    NoAcousticSubstepperDiagnostics()

function materialize_acoustic_substepper_diagnostics(grid, ::HorizontalPressureIncrementDiagnostics)
    return MaterializedHorizontalPressureIncrementDiagnostics((u = XFaceField(grid),
                                                             v = YFaceField(grid)),
                                                             (u = XFaceField(grid),
                                                              v = YFaceField(grid)),
                                                             (u = XFaceField(grid),
...
function materialize_acoustic_substepper_diagnostics(grid, diagnostics::BottomPressureModalDiagnostics)
    pressure_diagnostics =
        materialize_acoustic_substepper_diagnostics(grid, HorizontalPressureIncrementDiagnostics())
    theta_terms = (previous = CenterField(grid),
                   slow_tendency = CenterField(grid),
                   horizontal_divergence = CenterField(grid),
                   vertical_flux = CenterField(grid),
                   total = CenterField(grid))
    density_predictor = CenterField(grid)
    vertical_terms = (previous = ZFaceField(grid),
                      slow_tendency = ZFaceField(grid),
                      pressure_old = ZFaceField(grid),
                      pressure_predictor = ZFaceField(grid),
                      buoyancy_old = ZFaceField(grid),
                      buoyancy_predictor = ZFaceField(grid),
                      damping = ZFaceField(grid),
                      sponge = ZFaceField(grid),
                      total = ZFaceField(grid))
    slow_vertical_terms = (gn_rhow = ZFaceField(grid),
                           horizontal_slow_x = ZFaceField(grid),
                           horizontal_slow_y = ZFaceField(grid),
                           vertical_pressure = ZFaceField(grid),
                           horizontal_pressure = ZFaceField(grid),
                           buoyancy = ZFaceField(grid),
                           total = ZFaceField(grid))
    return MaterializedBottomPressureModalDiagnostics(pressure_diagnostics,
                                                      theta_terms,
                                                      density_predictor,
                                                      vertical_terms,
                                                      slow_vertical_terms,
                                                      collect(Int, diagnostics.modes),
                                                      collect(Int, diagnostics.levels),
                                                      BottomPressureModalSample[],
                                                      StepBTermModalSample[],
                                                      RKStageModalSample[])
end

struct AcousticSubstepper{N, FT, D, AD, US, CF, MP, TAV, GT, TS, DIAG}
    substeps :: N
    acoustic_cfl :: FT
    forward_weight :: FT
...
Adapt.adapt_structure(to, a::AcousticSubstepper) =
    AcousticSubstepper(a.substeps,
                       ...
                       adapt(to, a.diagnostics))

Adapt.adapt_structure(to, d::MaterializedHorizontalPressureIncrementDiagnostics) =
    MaterializedHorizontalPressureIncrementDiagnostics((u = adapt(to, d.momentum_increment.u),
                                                       v = adapt(to, d.momentum_increment.v)),
                                                       (u = adapt(to, d.frozen_momentum_increment.u),
                                                       v = adapt(to, d.frozen_momentum_increment.v)),
                                                       (u = adapt(to, d.perturbation_momentum_increment.u),
                                                        v = adapt(to, d.perturbation_momentum_increment.v)),
                                                       (u = adapt(to, d.horizontal_momentum_increment.u),
                                                        v = adapt(to, d.horizontal_momentum_increment.v)),
                                                       (u = adapt(to, d.terrain_momentum_increment.u),
                                                        v = adapt(to, d.terrain_momentum_increment.v)),
                                                       (u = adapt(to, d.frozen_horizontal_momentum_increment.u),
                                                        v = adapt(to, d.frozen_horizontal_momentum_increment.v)),
                                                       (u = adapt(to, d.frozen_terrain_momentum_increment.u),
                                                        v = adapt(to, d.frozen_terrain_momentum_increment.v)),
                                                       (u = adapt(to, d.perturbation_horizontal_momentum_increment.u),
                                                        v = adapt(to, d.perturbation_horizontal_momentum_increment.v)),
                                                       (u = adapt(to, d.perturbation_terrain_momentum_increment.u),
                                                        v = adapt(to, d.perturbation_terrain_momentum_increment.v)),
                                                       (u = adapt(to, d.ungated_horizontal_momentum_increment.u),
                                                        v = adapt(to, d.ungated_horizontal_momentum_increment.v)),
                                                       (u = adapt(to, d.ungated_terrain_momentum_increment.u),
                                                        v = adapt(to, d.ungated_terrain_momentum_increment.v)),
                                                       (u = adapt(to, d.slow_momentum_increment.u),
                                                        v = adapt(to, d.slow_momentum_increment.v)),
                                                       (u = adapt(to, d.initial_momentum_perturbation.u),
                                                        v = adapt(to, d.initial_momentum_perturbation.v)),
                                                       (u = adapt(to, d.final_momentum_perturbation.u),
                                                        v = adapt(to, d.final_momentum_perturbation.v)),
                                                       (u = adapt(to, d.exner_acceleration.u),
                                                        v = adapt(to, d.exner_acceleration.v)),
                                                       (u = adapt(to, d.frozen_exner_acceleration.u),
                                                        v = adapt(to, d.frozen_exner_acceleration.v)),
                                                       (u = adapt(to, d.perturbation_exner_acceleration.u),
                                                        v = adapt(to, d.perturbation_exner_acceleration.v)),
                                                       (u = adapt(to, d.horizontal_exner_acceleration.u),
                                                        v = adapt(to, d.horizontal_exner_acceleration.v)),
                                                       (u = adapt(to, d.terrain_exner_acceleration.u),
                                                        v = adapt(to, d.terrain_exner_acceleration.v)),
                                                       (u = adapt(to, d.frozen_horizontal_exner_acceleration.u),
                                                        v = adapt(to, d.frozen_horizontal_exner_acceleration.v)),
                                                       (u = adapt(to, d.frozen_terrain_exner_acceleration.u),
                                                        v = adapt(to, d.frozen_terrain_exner_acceleration.v)),
                                                       (u = adapt(to, d.perturbation_horizontal_exner_acceleration.u),
                                                        v = adapt(to, d.perturbation_horizontal_exner_acceleration.v)),
                                                       (u = adapt(to, d.perturbation_terrain_exner_acceleration.u),
                                                        v = adapt(to, d.perturbation_terrain_exner_acceleration.v)),
                                                       (u = adapt(to, d.ungated_horizontal_exner_acceleration.u),
                                                        v = adapt(to, d.ungated_horizontal_exner_acceleration.v)),
                                                       (u = adapt(to, d.ungated_terrain_exner_acceleration.u),
                                                        v = adapt(to, d.ungated_terrain_exner_acceleration.v)),
                                                       (u = adapt(to, d.nonlinear_pressure_acceleration.u),
                                                        v = adapt(to, d.nonlinear_pressure_acceleration.v)),
                                                       (u = adapt(to, d.nonlinear_horizontal_pressure_acceleration.u),
                                                        v = adapt(to, d.nonlinear_horizontal_pressure_acceleration.v)),
                                                       (u = adapt(to, d.nonlinear_terrain_pressure_acceleration.u),
                                                        v = adapt(to, d.nonlinear_terrain_pressure_acceleration.v)),
                                                       (u = adapt(to, d.predictor_pressure_acceleration.u),
                                                        v = adapt(to, d.predictor_pressure_acceleration.v)),
                                                       (u = adapt(to, d.predictor_horizontal_pressure_acceleration.u),
                                                        v = adapt(to, d.predictor_horizontal_pressure_acceleration.v)),
                                                       (u = adapt(to, d.predictor_terrain_pressure_acceleration.u),
                                                        v = adapt(to, d.predictor_terrain_pressure_acceleration.v)),
                                                       (u = adapt(to, d.recovered_pressure_acceleration.u),
                                                        v = adapt(to, d.recovered_pressure_acceleration.v)),
                                                       (u = adapt(to, d.recovered_horizontal_pressure_acceleration.u),
                                                        v = adapt(to, d.recovered_horizontal_pressure_acceleration.v)),
                                                       (u = adapt(to, d.recovered_terrain_pressure_acceleration.u),
                                                        v = adapt(to, d.recovered_terrain_pressure_acceleration.v)))

Adapt.adapt_structure(to, d::MaterializedBottomPressureModalDiagnostics) =
    MaterializedBottomPressureModalDiagnostics(adapt(to, d.pressure_diagnostics),
                                              adapt(to, d.theta_terms),
                                              adapt(to, d.density_predictor),
                                              adapt(to, d.vertical_terms),
                                              adapt(to, d.slow_vertical_terms),
                                              d.modes,
                                              d.levels,
                                              d.samples,
                                              d.step_b_samples,
...
function AcousticSubstepper(grid, split_explicit::SplitExplicitTimeDiscretization;
    ...
    diagnostics = materialize_acoustic_substepper_diagnostics(grid, split_explicit.diagnostics)
...
@inline function record_horizontal_pressure_increment!(::NoAcousticSubstepperDiagnostics,
                                                       arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                       Gⁿρu, Gⁿρv,
                                                       ρᴸ, θᴸ, γRᵐᴸ, apply_pressure_gradient,
                                                       Δt, pˢᵗ, κ, cᵖᵈ)
    return nothing
end

@inline reset_horizontal_pressure_increment!(::NoAcousticSubstepperDiagnostics) = nothing
@inline record_initial_horizontal_momentum_perturbation!(::NoAcousticSubstepperDiagnostics, substepper) = nothing
@inline record_final_horizontal_momentum_perturbation!(::NoAcousticSubstepperDiagnostics, substepper) = nothing
@inline record_pressure_state_acceleration!(::NoAcousticSubstepperDiagnostics,
                                            arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                            ρᴸ, θᴸ, γRᵐᴸ, Δt, fields) = nothing
@inline record_pre_predictor_modal_state!(::NoAcousticSubstepperDiagnostics,
                                          time, outer_iteration, stage_fraction,
                                          substep, substeps, ρθ′, Πᴸ, γRᵐᴸ,
                                          ρu′, ρw′, ρᴸ) = nothing
@inline record_step_b_term_modal_state!(::NoAcousticSubstepperDiagnostics,
                                        args...) = nothing

@inline record_step_b_term_modal_state!(::MaterializedHorizontalPressureIncrementDiagnostics,
                                        args...) = nothing

@inline record_slow_vertical_assembly_modal_state!(::NoAcousticSubstepperDiagnostics,
                                                   args...) = nothing

@inline record_slow_vertical_assembly_modal_state!(::MaterializedHorizontalPressureIncrementDiagnostics,
                                                   args...) = nothing

@inline record_slow_vertical_assembly_modal_state!(::MaterializedBottomPressureModalDiagnostics,
                                                   args...) = nothing
@inline record_rk_stage_modal_state!(::NoAcousticSubstepperDiagnostics,
                                     args...) = nothing
@inline record_rk_stage_modal_state!(::MaterializedHorizontalPressureIncrementDiagnostics,
                                     args...) = nothing
@inline record_rk_stage_perturbation_modal_state!(::NoAcousticSubstepperDiagnostics,
                                                  args...) = nothing
@inline record_rk_stage_perturbation_modal_state!(::MaterializedHorizontalPressureIncrementDiagnostics,
                                                  args...) = nothing
@inline record_predictor_pressure_state_acceleration!(::NoAcousticSubstepperDiagnostics,
                                                      arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                      ρᴸ, θᴸ, γRᵐᴸ, ρu′, ρw′, Δt,
                                                      time, outer_iteration, stage_fraction,
                                                      substep, substeps) = nothing
@inline record_recovered_pressure_state_acceleration!(::NoAcousticSubstepperDiagnostics,
                                                      arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                      ρᴸ, θᴸ, γRᵐᴸ, ρu′, ρw′, Δt,
                                                      time, outer_iteration, stage_fraction,
                                                      substep, substeps) = nothing

function reset_horizontal_pressure_increment!(diagnostics::MaterializedHorizontalPressureIncrementDiagnostics)
    fill!(parent(diagnostics.momentum_increment.u), 0)
    fill!(parent(diagnostics.momentum_increment.v), 0)
    fill!(parent(diagnostics.frozen_momentum_increment.u), 0)
    fill!(parent(diagnostics.frozen_momentum_increment.v), 0)
    fill!(parent(diagnostics.perturbation_momentum_increment.u), 0)
    fill!(parent(diagnostics.perturbation_momentum_increment.v), 0)
    fill!(parent(diagnostics.horizontal_momentum_increment.u), 0)
    fill!(parent(diagnostics.horizontal_momentum_increment.v), 0)
    fill!(parent(diagnostics.terrain_momentum_increment.u), 0)
    fill!(parent(diagnostics.terrain_momentum_increment.v), 0)
    fill!(parent(diagnostics.frozen_horizontal_momentum_increment.u), 0)
    fill!(parent(diagnostics.frozen_horizontal_momentum_increment.v), 0)
    fill!(parent(diagnostics.frozen_terrain_momentum_increment.u), 0)
    fill!(parent(diagnostics.frozen_terrain_momentum_increment.v), 0)
    fill!(parent(diagnostics.perturbation_horizontal_momentum_increment.u), 0)
    fill!(parent(diagnostics.perturbation_horizontal_momentum_increment.v), 0)
    fill!(parent(diagnostics.perturbation_terrain_momentum_increment.u), 0)
    fill!(parent(diagnostics.perturbation_terrain_momentum_increment.v), 0)
    fill!(parent(diagnostics.ungated_horizontal_momentum_increment.u), 0)
    fill!(parent(diagnostics.ungated_horizontal_momentum_increment.v), 0)
    fill!(parent(diagnostics.ungated_terrain_momentum_increment.u), 0)
    fill!(parent(diagnostics.ungated_terrain_momentum_increment.v), 0)
    fill!(parent(diagnostics.slow_momentum_increment.u), 0)
    fill!(parent(diagnostics.slow_momentum_increment.v), 0)
    fill!(parent(diagnostics.initial_momentum_perturbation.u), 0)
    fill!(parent(diagnostics.initial_momentum_perturbation.v), 0)
    fill!(parent(diagnostics.final_momentum_perturbation.u), 0)
    fill!(parent(diagnostics.final_momentum_perturbation.v), 0)
    fill!(parent(diagnostics.exner_acceleration.u), 0)
    fill!(parent(diagnostics.exner_acceleration.v), 0)
    fill!(parent(diagnostics.frozen_exner_acceleration.u), 0)
    fill!(parent(diagnostics.frozen_exner_acceleration.v), 0)
    fill!(parent(diagnostics.perturbation_exner_acceleration.u), 0)
    fill!(parent(diagnostics.perturbation_exner_acceleration.v), 0)
    fill!(parent(diagnostics.horizontal_exner_acceleration.u), 0)
    fill!(parent(diagnostics.horizontal_exner_acceleration.v), 0)
    fill!(parent(diagnostics.terrain_exner_acceleration.u), 0)
    fill!(parent(diagnostics.terrain_exner_acceleration.v), 0)
    fill!(parent(diagnostics.frozen_horizontal_exner_acceleration.u), 0)
    fill!(parent(diagnostics.frozen_horizontal_exner_acceleration.v), 0)
    fill!(parent(diagnostics.frozen_terrain_exner_acceleration.u), 0)
    fill!(parent(diagnostics.frozen_terrain_exner_acceleration.v), 0)
    fill!(parent(diagnostics.perturbation_horizontal_exner_acceleration.u), 0)
    fill!(parent(diagnostics.perturbation_horizontal_exner_acceleration.v), 0)
    fill!(parent(diagnostics.perturbation_terrain_exner_acceleration.u), 0)
    fill!(parent(diagnostics.perturbation_terrain_exner_acceleration.v), 0)
    fill!(parent(diagnostics.ungated_horizontal_exner_acceleration.u), 0)
    fill!(parent(diagnostics.ungated_horizontal_exner_acceleration.v), 0)
    fill!(parent(diagnostics.ungated_terrain_exner_acceleration.u), 0)
    fill!(parent(diagnostics.ungated_terrain_exner_acceleration.v), 0)
    fill!(parent(diagnostics.nonlinear_pressure_acceleration.u), 0)
    fill!(parent(diagnostics.nonlinear_pressure_acceleration.v), 0)
    fill!(parent(diagnostics.nonlinear_horizontal_pressure_acceleration.u), 0)
    fill!(parent(diagnostics.nonlinear_horizontal_pressure_acceleration.v), 0)
    fill!(parent(diagnostics.nonlinear_terrain_pressure_acceleration.u), 0)
    fill!(parent(diagnostics.nonlinear_terrain_pressure_acceleration.v), 0)
    fill!(parent(diagnostics.predictor_pressure_acceleration.u), 0)
    fill!(parent(diagnostics.predictor_pressure_acceleration.v), 0)
    fill!(parent(diagnostics.predictor_horizontal_pressure_acceleration.u), 0)
    fill!(parent(diagnostics.predictor_horizontal_pressure_acceleration.v), 0)
    fill!(parent(diagnostics.predictor_terrain_pressure_acceleration.u), 0)
    fill!(parent(diagnostics.predictor_terrain_pressure_acceleration.v), 0)
    fill!(parent(diagnostics.recovered_pressure_acceleration.u), 0)
    fill!(parent(diagnostics.recovered_pressure_acceleration.v), 0)
    fill!(parent(diagnostics.recovered_horizontal_pressure_acceleration.u), 0)
    fill!(parent(diagnostics.recovered_horizontal_pressure_acceleration.v), 0)
    fill!(parent(diagnostics.recovered_terrain_pressure_acceleration.u), 0)
    fill!(parent(diagnostics.recovered_terrain_pressure_acceleration.v), 0)
    return nothing
end

function record_initial_horizontal_momentum_perturbation!(diagnostics::MaterializedHorizontalPressureIncrementDiagnostics,
                                                          substepper)
    parent(diagnostics.initial_momentum_perturbation.u) .=
        parent(substepper.momentum_perturbation.u)
    parent(diagnostics.initial_momentum_perturbation.v) .=
        parent(substepper.momentum_perturbation.v)
    return nothing
end

function record_final_horizontal_momentum_perturbation!(diagnostics::MaterializedHorizontalPressureIncrementDiagnostics,
                                                        substepper)
    parent(diagnostics.final_momentum_perturbation.u) .=
        parent(substepper.momentum_perturbation.u)
    parent(diagnostics.final_momentum_perturbation.v) .=
        parent(substepper.momentum_perturbation.v)
    return nothing
end

function record_horizontal_pressure_increment!(diagnostics::MaterializedHorizontalPressureIncrementDiagnostics,
                                               arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                               Gⁿρu, Gⁿρv,
                                               ρᴸ, θᴸ, γRᵐᴸ, apply_pressure_gradient,
                                               Δt, pˢᵗ, κ, cᵖᵈ)
    launch!(arch, grid, :xyz, _accumulate_horizontal_pressure_increment!,
            diagnostics.momentum_increment.u,
            diagnostics.momentum_increment.v,
            diagnostics.frozen_momentum_increment.u,
            diagnostics.frozen_momentum_increment.v,
            diagnostics.perturbation_momentum_increment.u,
            diagnostics.perturbation_momentum_increment.v,
            diagnostics.horizontal_momentum_increment.u,
            diagnostics.horizontal_momentum_increment.v,
            diagnostics.terrain_momentum_increment.u,
            diagnostics.terrain_momentum_increment.v,
            diagnostics.frozen_horizontal_momentum_increment.u,
            diagnostics.frozen_horizontal_momentum_increment.v,
            diagnostics.frozen_terrain_momentum_increment.u,
            diagnostics.frozen_terrain_momentum_increment.v,
            diagnostics.perturbation_horizontal_momentum_increment.u,
            diagnostics.perturbation_horizontal_momentum_increment.v,
            diagnostics.perturbation_terrain_momentum_increment.u,
            diagnostics.perturbation_terrain_momentum_increment.v,
            diagnostics.ungated_horizontal_momentum_increment.u,
            diagnostics.ungated_horizontal_momentum_increment.v,
            diagnostics.ungated_terrain_momentum_increment.u,
            diagnostics.ungated_terrain_momentum_increment.v,
            diagnostics.slow_momentum_increment.u,
            diagnostics.slow_momentum_increment.v,
            diagnostics.exner_acceleration.u,
            diagnostics.exner_acceleration.v,
            diagnostics.frozen_exner_acceleration.u,
            diagnostics.frozen_exner_acceleration.v,
            diagnostics.perturbation_exner_acceleration.u,
            diagnostics.perturbation_exner_acceleration.v,
            diagnostics.horizontal_exner_acceleration.u,
            diagnostics.horizontal_exner_acceleration.v,
            diagnostics.terrain_exner_acceleration.u,
            diagnostics.terrain_exner_acceleration.v,
            diagnostics.frozen_horizontal_exner_acceleration.u,
            diagnostics.frozen_horizontal_exner_acceleration.v,
            diagnostics.frozen_terrain_exner_acceleration.u,
            diagnostics.frozen_terrain_exner_acceleration.v,
            diagnostics.perturbation_horizontal_exner_acceleration.u,
            diagnostics.perturbation_horizontal_exner_acceleration.v,
            diagnostics.perturbation_terrain_exner_acceleration.u,
            diagnostics.perturbation_terrain_exner_acceleration.v,
            diagnostics.ungated_horizontal_exner_acceleration.u,
            diagnostics.ungated_horizontal_exner_acceleration.v,
            diagnostics.ungated_terrain_exner_acceleration.u,
            diagnostics.ungated_terrain_exner_acceleration.v,
            diagnostics.nonlinear_pressure_acceleration.u,
            diagnostics.nonlinear_pressure_acceleration.v,
            diagnostics.nonlinear_horizontal_pressure_acceleration.u,
            diagnostics.nonlinear_horizontal_pressure_acceleration.v,
            diagnostics.nonlinear_terrain_pressure_acceleration.u,
            diagnostics.nonlinear_terrain_pressure_acceleration.v,
            grid, dynamics, Δτ, ρθ′, Πᴸ, p, Gⁿρu, Gⁿρv, ρᴸ, θᴸ, γRᵐᴸ,
            apply_pressure_gradient, Δt, pˢᵗ, κ, cᵖᵈ)
    return nothing
end

function record_pressure_state_acceleration!(::MaterializedHorizontalPressureIncrementDiagnostics,
                                             arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                             ρᴸ, θᴸ, γRᵐᴸ, Δt, fields)
    launch!(arch, grid, :xyz, _accumulate_pressure_state_acceleration!,
            fields.pressure.u, fields.pressure.v,
            fields.horizontal.u, fields.horizontal.v,
            fields.terrain.u, fields.terrain.v,
            grid, dynamics, Δτ, ρθ′, Πᴸ, p, ρᴸ, θᴸ, γRᵐᴸ, Δt)
    return nothing
end

function record_predictor_pressure_state_acceleration!(diagnostics::MaterializedHorizontalPressureIncrementDiagnostics,
                                                       arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                       ρᴸ, θᴸ, γRᵐᴸ, ρu′, ρw′, Δt,
                                                       time, outer_iteration, stage_fraction,
                                                       substep, substeps)
    fields = (pressure = diagnostics.predictor_pressure_acceleration,
              horizontal = diagnostics.predictor_horizontal_pressure_acceleration,
              terrain = diagnostics.predictor_terrain_pressure_acceleration)
    return record_pressure_state_acceleration!(diagnostics, arch, grid, dynamics, Δτ,
                                               ρθ′, Πᴸ, p, ρᴸ, θᴸ, γRᵐᴸ, Δt, fields)
end

function record_recovered_pressure_state_acceleration!(diagnostics::MaterializedHorizontalPressureIncrementDiagnostics,
                                                       arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                       ρᴸ, θᴸ, γRᵐᴸ, ρu′, ρw′, Δt,
                                                       time, outer_iteration, stage_fraction,
                                                       substep, substeps)
    fields = (pressure = diagnostics.recovered_pressure_acceleration,
              horizontal = diagnostics.recovered_horizontal_pressure_acceleration,
              terrain = diagnostics.recovered_terrain_pressure_acceleration)
    return record_pressure_state_acceleration!(diagnostics, arch, grid, dynamics, Δτ,
                                               ρθ′, Πᴸ, p, ρᴸ, θᴸ, γRᵐᴸ, Δt, fields)
end

reset_horizontal_pressure_increment!(diagnostics::MaterializedBottomPressureModalDiagnostics) =
    reset_horizontal_pressure_increment!(diagnostics.pressure_diagnostics)

record_initial_horizontal_momentum_perturbation!(diagnostics::MaterializedBottomPressureModalDiagnostics,
                                                 substepper) =
    record_initial_horizontal_momentum_perturbation!(diagnostics.pressure_diagnostics,
                                                     substepper)

record_final_horizontal_momentum_perturbation!(diagnostics::MaterializedBottomPressureModalDiagnostics,
                                               substepper) =
    record_final_horizontal_momentum_perturbation!(diagnostics.pressure_diagnostics,
                                                   substepper)

record_horizontal_pressure_increment!(diagnostics::MaterializedBottomPressureModalDiagnostics,
                                      arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                      Gⁿρu, Gⁿρv,
                                      ρᴸ, θᴸ, γRᵐᴸ, apply_pressure_gradient,
                                      Δt, pˢᵗ, κ, cᵖᵈ) =
    record_horizontal_pressure_increment!(diagnostics.pressure_diagnostics,
                                          arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                          Gⁿρu, Gⁿρv,
                                          ρᴸ, θᴸ, γRᵐᴸ, apply_pressure_gradient,
                                          Δt, pˢᵗ, κ, cᵖᵈ)

function record_pre_predictor_modal_state!(diagnostics::MaterializedBottomPressureModalDiagnostics,
                                           time, outer_iteration, stage_fraction,
                                           substep, substeps, ρθ′, Πᴸ, γRᵐᴸ,
                                           ρu′, ρw′, ρᴸ)
    record_bottom_pressure_modal_state!(diagnostics, :pre_predictor, time,
                                        outer_iteration, stage_fraction,
                                        substep, substeps, ρθ′, Πᴸ, γRᵐᴸ,
                                        ρu′, ρw′, ρᴸ)
    return nothing
end

@kernel function _record_step_b_terms!(theta_previous, theta_slow_tendency,
                                       theta_horizontal_divergence, theta_vertical_flux,
                                       theta_total, density_predictor,
                                       vertical_previous, vertical_slow_tendency,
                                       vertical_pressure_old, vertical_pressure_predictor,
                                       vertical_buoyancy_old, vertical_buoyancy_predictor,
                                       vertical_damping, vertical_sponge, vertical_total,
                                       ρ′, ρθ′, ρw′, ρu′, ρv′,
                                       grid, dynamics, Δτ, δτᵐ⁺, δτˢ⁻,
                                       Gˢρ, Gˢρθ, Gˢρw,
                                       thermodynamic_tendency_factor,
                                       θᴸ, Πᴸ, γRᵐᴸ, g, dˢ⁻, sponge)
    ...
    @inbounds begin
        for k in 1:Nz
            ...
            density_predictor[i, j, k] = ρ′[i, j, k] +
                                         Δτ * (Gˢρ[i, j, k] - ∇ʰ_M) +
                                         mass_vertical_flux
            theta_previous[i, j, k] = ρθ′[i, j, k]
            theta_slow_tendency[i, j, k] =
                Δτ * thermodynamic_tendency_factor * Gˢρθ[i, j, k]
            theta_horizontal_divergence[i, j, k] = - Δτ * ∇ʰ_θM
            theta_vertical_flux[i, j, k] = vertical_flux
            theta_total[i, j, k] = ρθ′[i, j, k] +
                                   theta_slow_tendency[i, j, k] +
                                   theta_horizontal_divergence[i, j, k] +
                                   theta_vertical_flux[i, j, k]
        end

    end
end

@kernel function _record_step_b_vertical_terms!(vertical_previous, vertical_slow_tendency,
                                                vertical_pressure_old, vertical_pressure_predictor,
                                                vertical_buoyancy_old, vertical_buoyancy_predictor,
                                                vertical_damping, vertical_sponge, vertical_total,
                                                density_predictor, theta_total,
                                                ρ′, ρθ′, ρw′,
                                                grid, dynamics, Δτ, δτᵐ⁺, δτˢ⁻,
                                                Gˢρw, Πᴸ, γRᵐᴸ, g, dˢ⁻,
                                                vertical_momentum_tendency_factor,
    ...
    @inbounds begin
        for k in 2:Nz
            ...
            ρ′ᶜᶜᶠ★  = ℑzᵃᵃᶠ(i, j, k, grid, density_predictor)
            ...
            vertical_previous[i, j, k] = ρw′[i, j, k]
            vertical_slow_tendency[i, j, k] =
                Δτ * vertical_momentum_tendency_factor * Gˢρw[i, j, k]
            vertical_pressure_old[i, j, k] = - δτˢ⁻ * ∂ζ_p′ˢ⁻
            vertical_pressure_predictor[i, j, k] = - δτᵐ⁺ * ∂ζ_p′★
            vertical_buoyancy_old[i, j, k] = - g * δτˢ⁻ * ρ′ᶜᶜᶠˢ⁻
            vertical_buoyancy_predictor[i, j, k] = - g * δτᵐ⁺ * ρ′ᶜᶜᶠ★
            vertical_damping[i, j, k] = - damp_force
            vertical_sponge[i, j, k] = - sponge_force
            vertical_total[i, j, k] =
                                      ρw′[i, j, k] +
                                      Δτ * vertical_momentum_tendency_factor * Gˢρw[i, j, k] -
                                      δτˢ⁻ * ∂ζ_p′ˢ⁻ - δτᵐ⁺ * ∂ζ_p′★ -
                                      g * δτˢ⁻ * ρ′ᶜᶜᶠˢ⁻ -
                                      g * δτᵐ⁺ * ρ′ᶜᶜᶠ★ -
                                      damp_force - sponge_force
        end

        vertical_previous[i, j, 1] = 0
        vertical_slow_tendency[i, j, 1] = 0
        vertical_pressure_old[i, j, 1] = 0
        vertical_pressure_predictor[i, j, 1] = 0
        vertical_buoyancy_old[i, j, 1] = 0
        vertical_buoyancy_predictor[i, j, 1] = 0
        vertical_damping[i, j, 1] = 0
        vertical_sponge[i, j, 1] = 0
        vertical_total[i, j, 1] = 0

        vertical_previous[i, j, Nz + 1] = 0
        vertical_slow_tendency[i, j, Nz + 1] = 0
        vertical_pressure_old[i, j, Nz + 1] = 0
        vertical_pressure_predictor[i, j, Nz + 1] = 0
        vertical_buoyancy_old[i, j, Nz + 1] = 0
        vertical_buoyancy_predictor[i, j, Nz + 1] = 0
        vertical_damping[i, j, Nz + 1] = 0
        vertical_sponge[i, j, Nz + 1] = 0
        vertical_total[i, j, Nz + 1] = 0
    end
end

function record_step_b_term_modal_state!(diagnostics::MaterializedBottomPressureModalDiagnostics,
                                         arch, grid, dynamics, Δτ, δτᵐ⁺, δτˢ⁻,
                                         ρ′, ρθ′, ρw′, ρu′, ρv′,
                                         Gˢρ, Gˢρθ, Gˢρw,
                                         thermodynamic_tendency_factor,
                                         vertical_momentum_tendency_factor,
                                         ...
                                         time, outer_iteration, stage_fraction,
                                         substep, substeps)
    launch!(arch, grid, :xy, _record_step_b_terms!,
            diagnostics.theta_terms.previous,
            diagnostics.theta_terms.slow_tendency,
            diagnostics.theta_terms.horizontal_divergence,
            diagnostics.theta_terms.vertical_flux,
            diagnostics.theta_terms.total,
            diagnostics.density_predictor,
            diagnostics.vertical_terms.previous,
            diagnostics.vertical_terms.slow_tendency,
            diagnostics.vertical_terms.pressure_old,
            diagnostics.vertical_terms.pressure_predictor,
            diagnostics.vertical_terms.buoyancy_old,
            diagnostics.vertical_terms.buoyancy_predictor,
            diagnostics.vertical_terms.damping,
            diagnostics.vertical_terms.sponge,
            diagnostics.vertical_terms.total,
            ρ′, ρθ′, ρw′, ρu′, ρv′,
            grid, dynamics, Δτ, δτᵐ⁺, δτˢ⁻,
            Gˢρ, Gˢρθ, Gˢρw,
            thermodynamic_tendency_factor,
    ...
    launch!(arch, grid, :xy, _record_step_b_vertical_terms!,
            diagnostics.vertical_terms.previous,
            diagnostics.vertical_terms.slow_tendency,
            diagnostics.vertical_terms.pressure_old,
            diagnostics.vertical_terms.pressure_predictor,
            ...
            grid, dynamics, Δτ, δτᵐ⁺, δτˢ⁻,
            Gˢρw, Πᴸ, γRᵐᴸ, g, dˢ⁻,
            vertical_momentum_tendency_factor, sponge)

    record_step_b_term_samples!(diagnostics, :theta_predictor, time,
                                outer_iteration, stage_fraction, substep, substeps,
                                diagnostics.theta_terms)
    record_step_b_term_samples!(diagnostics, :vertical_rhs, time,
                                outer_iteration, stage_fraction, substep, substeps,
                                diagnostics.vertical_terms)
    return nothing
end

function record_predictor_pressure_state_acceleration!(diagnostics::MaterializedBottomPressureModalDiagnostics,
                                                       arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                       ρᴸ, θᴸ, γRᵐᴸ, ρu′, ρw′, Δt,
                                                       time, outer_iteration, stage_fraction,
                                                       substep, substeps)
    record_predictor_pressure_state_acceleration!(diagnostics.pressure_diagnostics,
                                                  arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                  ρᴸ, θᴸ, γRᵐᴸ, ρu′, ρw′, Δt,
                                                  time, outer_iteration, stage_fraction,
                                                  substep, substeps)
    record_bottom_pressure_modal_state!(diagnostics, :predictor, time,
                                        outer_iteration, stage_fraction,
                                        substep, substeps, ρθ′, Πᴸ, γRᵐᴸ,
                                        ρu′, ρw′, ρᴸ)
    return nothing
...
function record_recovered_pressure_state_acceleration!(diagnostics::MaterializedBottomPressureModalDiagnostics,
                                                       arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                       ρᴸ, θᴸ, γRᵐᴸ, ρu′, ρw′, Δt,
                                                       time, outer_iteration, stage_fraction,
                                                       substep, substeps)
    record_recovered_pressure_state_acceleration!(diagnostics.pressure_diagnostics,
                                                  arch, grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                  ρᴸ, θᴸ, γRᵐᴸ, ρu′, ρw′, Δt,
                                                  time, outer_iteration, stage_fraction,
                                                  substep, substeps)
    record_bottom_pressure_modal_state!(diagnostics, :recovered, time,
                                        outer_iteration, stage_fraction,
                                        substep, substeps, ρθ′, Πᴸ, γRᵐᴸ,
                                        ρu′, ρw′, ρᴸ)
    return nothing
...
function diagnostic_matrix(field)
    array = Array(interior(field))
    return ndims(array) == 3 ? dropdims(array; dims = 2) : array
end

function diagnostic_modal_coefficient(values, mode)
    count = length(values)
    ...
    return complex(real_part, imaginary_part) / count
end

phase_degrees(value) = 180 / π * angle(value)

@inline function rk_stage_index(stage_fraction)
    isapprox(stage_fraction, 1 / 3; atol = 1e-12) && return 1
    isapprox(stage_fraction, 1 / 2; atol = 1e-12) && return 2
    isapprox(stage_fraction, 1; atol = 1e-12) && return 3
    return 0
end

function diagnostic_pressure_perturbation_matrix(dynamics)
    pressure_values = diagnostic_matrix(dynamics.pressure)
    reference_pressure = dynamics.terrain_reference_pressure
    reference_pressure === nothing && return Float64.(pressure_values)
    return Float64.(pressure_values .- diagnostic_matrix(reference_pressure))
end

function record_rk_stage_modal_state!(diagnostics::MaterializedBottomPressureModalDiagnostics,
                                      model, state, time, outer_iteration,
                                      stage_fraction)
    compute_contravariant_velocity!(model)

    density_values = diagnostic_matrix(model.dynamics.density)
    χ_field = thermodynamic_density(model.formulation)
    θ_values = Float64.(diagnostic_matrix(χ_field) ./ density_values)
    pressure_values = diagnostic_pressure_perturbation_matrix(model.dynamics)
    ρu_values = Float64.(diagnostic_matrix(model.momentum.ρu))
    ρw_values = Float64.(diagnostic_matrix(model.momentum.ρw))
    u_values = Float64.(diagnostic_matrix(model.velocities.u))
    w_tilde_field = model.dynamics.contravariant_vertical_velocity
    w_tilde_values = w_tilde_field === nothing ?
                     Float64.(diagnostic_matrix(model.velocities.w)) :
                     Float64.(diagnostic_matrix(w_tilde_field))
    stage_index = rk_stage_index(stage_fraction)

    for k in diagnostics.levels
        for mode in diagnostics.modes
            θ_coefficient = k <= size(θ_values, 2) ?
                            diagnostic_modal_coefficient(θ_values[:, k], mode) :
                            complex(NaN, NaN)
            pressure_coefficient = k <= size(pressure_values, 2) ?
                                   diagnostic_modal_coefficient(pressure_values[:, k], mode) :
                                   complex(NaN, NaN)
            ρu_coefficient = k <= size(ρu_values, 2) ?
                             diagnostic_modal_coefficient(ρu_values[:, k], mode) :
                             complex(NaN, NaN)
            ρw_coefficient = k <= size(ρw_values, 2) ?
                             diagnostic_modal_coefficient(ρw_values[:, k], mode) :
                             complex(NaN, NaN)
            u_coefficient = k <= size(u_values, 2) ?
                            diagnostic_modal_coefficient(u_values[:, k], mode) :
                            complex(NaN, NaN)
            w_tilde_coefficient = k <= size(w_tilde_values, 2) ?
                                  diagnostic_modal_coefficient(w_tilde_values[:, k], mode) :
                                  complex(NaN, NaN)

            push!(diagnostics.rk_stage_samples,
                  RKStageModalSample(Float64(time),
                                     Int(outer_iteration),
                                     Int(stage_index),
                                     Float64(stage_fraction),
                                     state,
                                     Int(k),
                                     Int(mode),
                                     abs(θ_coefficient),
                                     phase_degrees(θ_coefficient),
                                     abs(pressure_coefficient),
                                     phase_degrees(pressure_coefficient),
                                     abs(ρu_coefficient),
                                     phase_degrees(ρu_coefficient),
                                     abs(ρw_coefficient),
                                     phase_degrees(ρw_coefficient),
                                     abs(u_coefficient),
                                     phase_degrees(u_coefficient),
                                     abs(w_tilde_coefficient),
                                     phase_degrees(w_tilde_coefficient)))
        end
    end
...
function record_rk_stage_perturbation_modal_state!(diagnostics::MaterializedBottomPressureModalDiagnostics,
                                                   model, substepper, state,
                                                   time, outer_iteration,
                                                   stage_fraction)
    compute_contravariant_velocity!(model)

    density_base = diagnostic_matrix(model.dynamics.density)
    density_values = Float64.(density_base .+
                              diagnostic_matrix(substepper.density_perturbation))

    χ_field = thermodynamic_density(model.formulation)
    χ_values = Float64.(diagnostic_matrix(χ_field) .+
                        diagnostic_matrix(substepper.density_potential_temperature_perturbation))
    θ_values = χ_values ./ density_values

    base_pressure_values = diagnostic_pressure_perturbation_matrix(model.dynamics)
    perturbation_pressure_values =
        diagnostic_matrix(substepper.linearization_gamma_R_mixture) .*
        diagnostic_matrix(substepper.linearization_exner) .*
        diagnostic_matrix(substepper.density_potential_temperature_perturbation)
    pressure_values = Float64.(base_pressure_values .+ perturbation_pressure_values)

    ρu_values = Float64.(diagnostic_matrix(model.momentum.ρu) .+
                         diagnostic_matrix(substepper.momentum_perturbation.u))
    vertical_transport_momentum = model.dynamics.contravariant_vertical_momentum
    vertical_momentum_base = vertical_transport_momentum === nothing ?
                             diagnostic_matrix(model.momentum.ρw) :
                             diagnostic_matrix(vertical_transport_momentum)
    ρw_values = Float64.(vertical_momentum_base .+
                         diagnostic_matrix(substepper.momentum_perturbation.w))
    u_values = similar(ρu_values)
    w_tilde_values = similar(ρw_values)

    for k in axes(u_values, 2)
        ρ_face = diagnostic_xface_density(density_values, k, size(u_values, 1))
        u_values[:, k] .= ρu_values[:, k] ./ ρ_face
    end

    for k in axes(w_tilde_values, 2)
        ρ_face = diagnostic_zface_density(density_values, k)
        w_tilde_values[:, k] .= ρw_values[:, k] ./ ρ_face
    end
...
function diagnostic_xface_density(density_values, k, count)
    Nx = size(density_values, 1)
    values = Vector{Float64}(undef, count)

    for i = 1:count
        left = mod1(i - 1, Nx)
        right = mod1(i, Nx)
        values[i] = 0.5 * (density_values[left, k] + density_values[right, k])
    end

    return values
end

function diagnostic_zface_density(density_values, k)
    Nz = size(density_values, 2)

    if k <= 1
        return Float64.(density_values[:, 1])
    elseif k > Nz
        return Float64.(density_values[:, Nz])
    else
        return Float64.(0.5 .* (density_values[:, k - 1] .+ density_values[:, k]))
    end
end

function record_bottom_pressure_modal_state!(diagnostics, state, time,
                                             outer_iteration, stage_fraction, substep,
                                             substeps, ρθ′, Πᴸ, γRᵐᴸ, ρu′, ρw′, ρᴸ)
    ρθ′_values = diagnostic_matrix(ρθ′)
    Πᴸ_values = diagnostic_matrix(Πᴸ)
    γRᵐᴸ_values = diagnostic_matrix(γRᵐᴸ)
    ρu′_values = ρu′ === nothing ? nothing : diagnostic_matrix(ρu′)
    ρw′_values = ρw′ === nothing ? nothing : diagnostic_matrix(ρw′)
    ρᴸ_values = ρᴸ === nothing ? nothing : diagnostic_matrix(ρᴸ)
    Nz = size(ρθ′_values, 2)

    for k in diagnostics.levels
        1 <= k <= Nz || continue

        theta_values = Float64.(ρθ′_values[:, k])
        pressure_values = Float64.(γRᵐᴸ_values[:, k] .* Πᴸ_values[:, k] .* ρθ′_values[:, k])
        ρu_values = ρu′_values === nothing ? Float64[] : Float64.(ρu′_values[:, k])
        ρw_values = ρw′_values === nothing ? Float64[] : Float64.(ρw′_values[:, k])
        u_values = ρu′_values === nothing || ρᴸ_values === nothing ? Float64[] :
                   ρu_values ./ diagnostic_xface_density(ρᴸ_values, k, length(ρu_values))
        w_tilde_values = ρw′_values === nothing || ρᴸ_values === nothing ? Float64[] :
                         ρw_values ./ diagnostic_zface_density(ρᴸ_values, k)

        for mode in diagnostics.modes
            theta_coefficient = diagnostic_modal_coefficient(theta_values, mode)
            pressure_coefficient = diagnostic_modal_coefficient(pressure_values, mode)
            ρu_coefficient = isempty(ρu_values) ? complex(NaN, NaN) :
                             diagnostic_modal_coefficient(ρu_values, mode)
            ρw_coefficient = isempty(ρw_values) ? complex(NaN, NaN) :
                             diagnostic_modal_coefficient(ρw_values, mode)
            u_coefficient = isempty(u_values) ? complex(NaN, NaN) :
                            diagnostic_modal_coefficient(u_values, mode)
            w_tilde_coefficient = isempty(w_tilde_values) ? complex(NaN, NaN) :
                                  diagnostic_modal_coefficient(w_tilde_values, mode)

            push!(diagnostics.samples,
                  BottomPressureModalSample(Float64(time),
                                            Int(outer_iteration),
                                            Float64(stage_fraction),
                                            Int(substep),
                                            Int(substeps),
                                            ...
                                            Int(k),
                                            Int(mode),
                                            abs(theta_coefficient),
                                            phase_degrees(theta_coefficient),
                                            abs(pressure_coefficient),
                                            phase_degrees(pressure_coefficient),
                                            abs(ρu_coefficient),
                                            phase_degrees(ρu_coefficient),
...
function record_step_b_term_samples!(diagnostics, variable, time,
                                     outer_iteration, stage_fraction, substep, substeps,
                                     terms)
    for name in propertynames(terms)
        values = diagnostic_matrix(getproperty(terms, name))
        Nz = size(values, 2)

        for k in diagnostics.levels
            1 <= k <= Nz || continue
            term_values = Float64.(values[:, k])

            for mode in diagnostics.modes
                coefficient = diagnostic_modal_coefficient(term_values, mode)
                push!(diagnostics.step_b_samples,
                      StepBTermModalSample(Float64(time),
                                           Int(outer_iteration),
                                           Float64(stage_fraction),
                                           Int(substep),
                                           Int(substeps),
...
@kernel function _accumulate_pressure_state_acceleration!(u_pressure_acceleration,
                                                          v_pressure_acceleration,
                                                          u_horizontal_pressure_acceleration,
                                                          v_horizontal_pressure_acceleration,
                                                          u_terrain_pressure_acceleration,
                                                          v_terrain_pressure_acceleration,
                                                          grid, dynamics, Δτ, ρθ′, Πᴸ, p,
                                                          ρᴸ, θᴸ, γRᵐᴸ, Δt)
    i, j, k = @index(Global, NTuple)

    @inbounds begin
        ∂x_pᴸ_horizontal, ∂x_pᴸ_terrain =
            acoustic_x_pressure_gradient_components(i, j, k, grid, dynamics, p)
        ∂y_pᴸ_horizontal, ∂y_pᴸ_terrain =
            acoustic_y_pressure_gradient_components(i, j, k, grid, dynamics, p)
        ∂x_p′_horizontal, ∂x_p′_terrain =
            acoustic_x_linearized_pressure_gradient_components(i, j, k, grid,
                                                               dynamics, ρθ′, Πᴸ, γRᵐᴸ)
        ∂y_p′_horizontal, ∂y_p′_terrain =
            acoustic_y_linearized_pressure_gradient_components(i, j, k, grid,
                                                               dynamics, ρθ′, Πᴸ, γRᵐᴸ)

        ∂x_p_horizontal = ∂x_pᴸ_horizontal + ∂x_p′_horizontal
        ∂y_p_horizontal = ∂y_pᴸ_horizontal + ∂y_p′_horizontal
        ∂x_p_terrain = ∂x_pᴸ_terrain + ∂x_p′_terrain
        ∂y_p_terrain = ∂y_pᴸ_terrain + ∂y_p′_terrain
        ...
        u_pressure_acceleration[i, j, k] -= weight * (∂x_p_horizontal + ∂x_p_terrain) / ρ̂ᶠᶜᶜ
        v_pressure_acceleration[i, j, k] -= weight * (∂y_p_horizontal + ∂y_p_terrain) / ρ̂ᶜᶠᶜ
        u_horizontal_pressure_acceleration[i, j, k] -= weight * ∂x_p_horizontal / ρ̂ᶠᶜᶜ
        v_horizontal_pressure_acceleration[i, j, k] -= weight * ∂y_p_horizontal / ρ̂ᶜᶠᶜ
        u_terrain_pressure_acceleration[i, j, k] -= weight * ∂x_p_terrain / ρ̂ᶠᶜᶜ
        v_terrain_pressure_acceleration[i, j, k] -= weight * ∂y_p_terrain / ρ̂ᶜᶠᶜ
    end
end

@kernel function _accumulate_horizontal_pressure_increment!(ρu_pressure_increment,
                                                            ρv_pressure_increment,
                                                            ρu_frozen_pressure_increment,
                                                            ρv_frozen_pressure_increment,
                                                            ρu_perturbation_pressure_increment,
                                                            ρv_perturbation_pressure_increment,
                                                            ρu_horizontal_pressure_increment,
                                                            ρv_horizontal_pressure_increment,
                                                            ρu_terrain_pressure_increment,
                                                            ρv_terrain_pressure_increment,
                                                            ρu_frozen_horizontal_pressure_increment,
                                                            ρv_frozen_horizontal_pressure_increment,
                                                            ρu_frozen_terrain_pressure_increment,
                                                            ρv_frozen_terrain_pressure_increment,
                                                            ρu_perturbation_horizontal_pressure_increment,
                                                            ρv_perturbation_horizontal_pressure_increment,
                                                            ρu_perturbation_terrain_pressure_increment,
                                                            ρv_perturbation_terrain_pressure_increment,
                                                            ρu_ungated_horizontal_pressure_increment,
                                                            ρv_ungated_horizontal_pressure_increment,
                                                            ρu_ungated_terrain_pressure_increment,
                                                            ρv_ungated_terrain_pressure_increment,
                                                            ρu_slow_increment,
                                                            ρv_slow_increment,
                                                            u_exner_acceleration,
                                                            v_exner_acceleration,
                                                            u_frozen_exner_acceleration,
                                                            v_frozen_exner_acceleration,
                                                            u_perturbation_exner_acceleration,
                                                            v_perturbation_exner_acceleration,
                                                            u_horizontal_exner_acceleration,
                                                            v_horizontal_exner_acceleration,
                                                            u_terrain_exner_acceleration,
                                                            v_terrain_exner_acceleration,
                                                            u_frozen_horizontal_exner_acceleration,
                                                            v_frozen_horizontal_exner_acceleration,
                                                            u_frozen_terrain_exner_acceleration,
                                                            v_frozen_terrain_exner_acceleration,
                                                            u_perturbation_horizontal_exner_acceleration,
                                                            v_perturbation_horizontal_exner_acceleration,
                                                            u_perturbation_terrain_exner_acceleration,
                                                            v_perturbation_terrain_exner_acceleration,
                                                            u_ungated_horizontal_exner_acceleration,
                                                            v_ungated_horizontal_exner_acceleration,
                                                            u_ungated_terrain_exner_acceleration,
                                                            v_ungated_terrain_exner_acceleration,
                                                            u_nonlinear_pressure_acceleration,
                                                            v_nonlinear_pressure_acceleration,
                                                            u_nonlinear_horizontal_pressure_acceleration,
                                                            v_nonlinear_horizontal_pressure_acceleration,
                                                            u_nonlinear_terrain_pressure_acceleration,
                                                            v_nonlinear_terrain_pressure_acceleration,
                                                            grid, dynamics, Δτ,
                                                            ρθ′, Πᴸ, p, Gⁿρu, Gⁿρv, ρᴸ, θᴸ, γRᵐᴸ,
                                                            apply_pressure_gradient, Δt,
                                                            pˢᵗ, κ, cᵖᵈ)
    i, j, k = @index(Global, NTuple)
    ...
    @inbounds begin
        ...
        ∂x_p = ∂x_pᴸ + perturbation_pressure_gradient_factor * ∂x_p′
        ∂y_p = ∂y_pᴸ + perturbation_pressure_gradient_factor * ∂y_p′
        ∂x_p′_active = perturbation_pressure_gradient_factor * ∂x_p′
        ∂y_p′_active = perturbation_pressure_gradient_factor * ∂y_p′
        ∂x_p′_horizontal_active = perturbation_pressure_gradient_factor * ∂x_p′_horizontal
        ∂y_p′_horizontal_active = perturbation_pressure_gradient_factor * ∂y_p′_horizontal
        ∂x_p′_terrain_active = perturbation_pressure_gradient_factor * ∂x_p′_terrain
        ∂y_p′_terrain_active = perturbation_pressure_gradient_factor * ∂y_p′_terrain
        ∂x_p_horizontal = ∂x_pᴸ_horizontal + ∂x_p′_horizontal_active
        ∂y_p_horizontal = ∂y_pᴸ_horizontal + ∂y_p′_horizontal_active
        ∂x_p_terrain = ∂x_pᴸ_terrain + ∂x_p′_terrain_active
        ∂y_p_terrain = ∂y_pᴸ_terrain + ∂y_p′_terrain_active
        ∂x_p_ungated_horizontal = ∂x_pᴸ_horizontal + ∂x_p′_horizontal
        ∂y_p_ungated_horizontal = ∂y_pᴸ_horizontal + ∂y_p′_horizontal
        ∂x_p_ungated_terrain = ∂x_pᴸ_terrain + ∂x_p′_terrain
        ∂y_p_ungated_terrain = ∂y_pᴸ_terrain + ∂y_p′_terrain

        ρu_pressure_increment[i, j, k] -= Δτ * ∂x_p
        ρv_pressure_increment[i, j, k] -= Δτ * ∂y_p
        ρu_frozen_pressure_increment[i, j, k] -= Δτ * ∂x_pᴸ
        ρv_frozen_pressure_increment[i, j, k] -= Δτ * ∂y_pᴸ
        ρu_perturbation_pressure_increment[i, j, k] -= Δτ * ∂x_p′_active
        ρv_perturbation_pressure_increment[i, j, k] -= Δτ * ∂y_p′_active
        ρu_horizontal_pressure_increment[i, j, k] -= Δτ * ∂x_p_horizontal
        ρv_horizontal_pressure_increment[i, j, k] -= Δτ * ∂y_p_horizontal
        ρu_terrain_pressure_increment[i, j, k] -= Δτ * ∂x_p_terrain
        ρv_terrain_pressure_increment[i, j, k] -= Δτ * ∂y_p_terrain
        ρu_frozen_horizontal_pressure_increment[i, j, k] -= Δτ * ∂x_pᴸ_horizontal
        ρv_frozen_horizontal_pressure_increment[i, j, k] -= Δτ * ∂y_pᴸ_horizontal
        ρu_frozen_terrain_pressure_increment[i, j, k] -= Δτ * ∂x_pᴸ_terrain
        ρv_frozen_terrain_pressure_increment[i, j, k] -= Δτ * ∂y_pᴸ_terrain
        ρu_perturbation_horizontal_pressure_increment[i, j, k] -= Δτ * ∂x_p′_horizontal_active
        ρv_perturbation_horizontal_pressure_increment[i, j, k] -= Δτ * ∂y_p′_horizontal_active
        ρu_perturbation_terrain_pressure_increment[i, j, k] -= Δτ * ∂x_p′_terrain_active
        ρv_perturbation_terrain_pressure_increment[i, j, k] -= Δτ * ∂y_p′_terrain_active
        ρu_ungated_horizontal_pressure_increment[i, j, k] -= Δτ * ∂x_p_ungated_horizontal
        ρv_ungated_horizontal_pressure_increment[i, j, k] -= Δτ * ∂y_p_ungated_horizontal
        ρu_ungated_terrain_pressure_increment[i, j, k] -= Δτ * ∂x_p_ungated_terrain
        ρv_ungated_terrain_pressure_increment[i, j, k] -= Δτ * ∂y_p_ungated_terrain
        ρu_slow_increment[i, j, k] += Δτ * Gⁿρu[i, j, k]
        ρv_slow_increment[i, j, k] += Δτ * Gⁿρv[i, j, k]

        (u_exner_total, u_exner_frozen, u_exner_perturbation,
         u_exner_horizontal, u_exner_terrain,
         u_exner_frozen_horizontal, u_exner_frozen_terrain,
         u_exner_perturbation_horizontal, u_exner_perturbation_terrain) =
            cm1_style_x_exner_pressure_acceleration_components(i, j, k, grid, dynamics,
                                                               ρθ′, Πᴸ, p, ρᴸ, θᴸ, γRᵐᴸ,
                                                               apply_pressure_gradient,
                                                               pˢᵗ, κ, cᵖᵈ)
        (v_exner_total, v_exner_frozen, v_exner_perturbation,
         v_exner_horizontal, v_exner_terrain,
         v_exner_frozen_horizontal, v_exner_frozen_terrain,
         v_exner_perturbation_horizontal, v_exner_perturbation_terrain) =
            cm1_style_y_exner_pressure_acceleration_components(i, j, k, grid, dynamics,
                                                               ρθ′, Πᴸ, p, ρᴸ, θᴸ, γRᵐᴸ,
                                                               apply_pressure_gradient,
                                                               pˢᵗ, κ, cᵖᵈ)
        (_, _, _,
         u_exner_ungated_horizontal, u_exner_ungated_terrain,
         _, _, _, _) =
            cm1_style_x_exner_pressure_acceleration_components(i, j, k, grid, dynamics,
                                                               ρθ′, Πᴸ, p, ρᴸ, θᴸ, γRᵐᴸ,
                                                               true,
                                                               pˢᵗ, κ, cᵖᵈ)
        (_, _, _,
         v_exner_ungated_horizontal, v_exner_ungated_terrain,
         _, _, _, _) =
            cm1_style_y_exner_pressure_acceleration_components(i, j, k, grid, dynamics,
                                                               ρθ′, Πᴸ, p, ρᴸ, θᴸ, γRᵐᴸ,
                                                               true,
                                                               pˢᵗ, κ, cᵖᵈ)
        ∂x_p_nonlinear_horizontal, ∂x_p_nonlinear_terrain =
            acoustic_x_full_dry_pressure_gradient_components(i, j, k, grid, dynamics,
                                                             ρθ′, ρᴸ, θᴸ, pˢᵗ, κ, cᵖᵈ)
        ∂y_p_nonlinear_horizontal, ∂y_p_nonlinear_terrain =
            acoustic_y_full_dry_pressure_gradient_components(i, j, k, grid, dynamics,
                                                             ρθ′, ρᴸ, θᴸ, pˢᵗ, κ, cᵖᵈ)
        ∂x_p_nonlinear = ∂x_p_nonlinear_horizontal + ∂x_p_nonlinear_terrain
        ∂y_p_nonlinear = ∂y_p_nonlinear_horizontal + ∂y_p_nonlinear_terrain

        exner_weight = Δτ / Δt
        u_exner_acceleration[i, j, k] += exner_weight * u_exner_total
        v_exner_acceleration[i, j, k] += exner_weight * v_exner_total
        u_frozen_exner_acceleration[i, j, k] += exner_weight * u_exner_frozen
        v_frozen_exner_acceleration[i, j, k] += exner_weight * v_exner_frozen
        u_perturbation_exner_acceleration[i, j, k] += exner_weight * u_exner_perturbation
        v_perturbation_exner_acceleration[i, j, k] += exner_weight * v_exner_perturbation
        u_horizontal_exner_acceleration[i, j, k] += exner_weight * u_exner_horizontal
        v_horizontal_exner_acceleration[i, j, k] += exner_weight * v_exner_horizontal
        u_terrain_exner_acceleration[i, j, k] += exner_weight * u_exner_terrain
        v_terrain_exner_acceleration[i, j, k] += exner_weight * v_exner_terrain
        u_frozen_horizontal_exner_acceleration[i, j, k] += exner_weight * u_exner_frozen_horizontal
        v_frozen_horizontal_exner_acceleration[i, j, k] += exner_weight * v_exner_frozen_horizontal
        u_frozen_terrain_exner_acceleration[i, j, k] += exner_weight * u_exner_frozen_terrain
        v_frozen_terrain_exner_acceleration[i, j, k] += exner_weight * v_exner_frozen_terrain
        u_perturbation_horizontal_exner_acceleration[i, j, k] += exner_weight * u_exner_perturbation_horizontal
        v_perturbation_horizontal_exner_acceleration[i, j, k] += exner_weight * v_exner_perturbation_horizontal
        u_perturbation_terrain_exner_acceleration[i, j, k] += exner_weight * u_exner_perturbation_terrain
        v_perturbation_terrain_exner_acceleration[i, j, k] += exner_weight * v_exner_perturbation_terrain
        u_ungated_horizontal_exner_acceleration[i, j, k] += exner_weight * u_exner_ungated_horizontal
        v_ungated_horizontal_exner_acceleration[i, j, k] += exner_weight * v_exner_ungated_horizontal
        u_ungated_terrain_exner_acceleration[i, j, k] += exner_weight * u_exner_ungated_terrain
        v_ungated_terrain_exner_acceleration[i, j, k] += exner_weight * v_exner_ungated_terrain
        ρᶠᶜᶜ = ℑxᶠᵃᵃ(i, j, k, grid, ρᴸ)
        ρᶜᶠᶜ = ℑyᵃᶠᵃ(i, j, k, grid, ρᴸ)
        ρ̂ᶠᶜᶜ = ifelse(ρᶠᶜᶜ == 0, one(ρᶠᶜᶜ), ρᶠᶜᶜ)
        ρ̂ᶜᶠᶜ = ifelse(ρᶜᶠᶜ == 0, one(ρᶜᶠᶜ), ρᶜᶠᶜ)
        u_nonlinear_pressure_acceleration[i, j, k] -= exner_weight * ∂x_p_nonlinear / ρ̂ᶠᶜᶜ
        v_nonlinear_pressure_acceleration[i, j, k] -= exner_weight * ∂y_p_nonlinear / ρ̂ᶜᶠᶜ
        u_nonlinear_horizontal_pressure_acceleration[i, j, k] -=
            exner_weight * ∂x_p_nonlinear_horizontal / ρ̂ᶠᶜᶜ
        v_nonlinear_horizontal_pressure_acceleration[i, j, k] -=
            exner_weight * ∂y_p_nonlinear_horizontal / ρ̂ᶜᶠᶜ
        u_nonlinear_terrain_pressure_acceleration[i, j, k] -=
            exner_weight * ∂x_p_nonlinear_terrain / ρ̂ᶠᶜᶜ
        v_nonlinear_terrain_pressure_acceleration[i, j, k] -=
            exner_weight * ∂y_p_nonlinear_terrain / ρ̂ᶜᶠᶜ
    end
end
...
function acoustic_rk3_substep_loop!(model, substepper, Δt, β_stage, Uᴸ)
    ...
    reset_horizontal_pressure_increment!(substepper.diagnostics)
    ...
    record_slow_vertical_assembly_modal_state!(substepper.diagnostics,
                                               substepper, model,
                                               model.clock.time,
                                               model.clock.iteration,
                                               β_stage)
    record_rk_stage_modal_state!(substepper.diagnostics, model, :stage_entry,
                                 model.clock.time, model.clock.iteration,
                                 β_stage)
    ...
    record_rk_stage_perturbation_modal_state!(substepper.diagnostics,
                                              model, substepper, :post_rewind,
                                              model.clock.time,
                                              model.clock.iteration,
                                              β_stage)
    record_initial_horizontal_momentum_perturbation!(substepper.diagnostics, substepper)
    ...
    for substep in 1:Nτ
        ...
        record_horizontal_pressure_increment!(substepper.diagnostics,
                                              arch, grid, model.dynamics, Δτ,
                                              substepper.density_potential_temperature_perturbation,
                                              substepper.linearization_exner,
                                              model.dynamics.pressure,
                                              Gⁿ.ρu, Gⁿ.ρv,
        ...
        record_pre_predictor_modal_state!(substepper.diagnostics,
                                          model.clock.time,
                                          model.clock.iteration,
                                          β_stage,
                                          substep,
        ...
        record_step_b_term_modal_state!(substepper.diagnostics,
                                        arch, grid, model.dynamics, Δτ, δτᵐ⁺, δτˢ⁻,
                                        substepper.density_perturbation,
                                        substepper.density_potential_temperature_perturbation,
                                        substepper.momentum_perturbation.w,
                                        substepper.momentum_perturbation.u,
        ...
        if substep == Nτ
            record_rk_stage_perturbation_modal_state!(substepper.diagnostics,
                                                      model, substepper, :post_vertical_rhs,
                                                      model.clock.time,
                                                      model.clock.iteration,
                                                      β_stage)
        end

        record_predictor_pressure_state_acceleration!(substepper.diagnostics,
                                                      arch, grid, model.dynamics, Δτ,
                                                      substepper.density_potential_temperature_predictor,
                                                      substepper.linearization_exner,
                                                      model.dynamics.pressure,
        ...
        if substep == Nτ
            record_rk_stage_perturbation_modal_state!(substepper.diagnostics,
                                                      model, substepper, :post_vertical_solve,
                                                      model.clock.time,
                                                      model.clock.iteration,
                                                      β_stage)
        end
        ...
        record_recovered_pressure_state_acceleration!(substepper.diagnostics,
                                                      arch, grid, model.dynamics, Δτ,
                                                      substepper.density_potential_temperature_perturbation,
                                                      substepper.linearization_exner,
                                                      model.dynamics.pressure,
    ...
    record_final_horizontal_momentum_perturbation!(substepper.diagnostics, substepper)
    ...
    record_rk_stage_modal_state!(substepper.diagnostics, model, :stage_exit,
                                 model.clock.time, model.clock.iteration,
                                 β_stage)
