...
@inline _acoustic_x_pgf_correction(i, j, k, grid, π̃′, ::Nothing) = zero(grid)
@inline _acoustic_y_pgf_correction(i, j, k, grid, π̃′, ::Nothing) = zero(grid)

# Kinematic bottom BC: w_bottom = u·(∂h/∂x) + v·(∂h/∂y)
# Returns 0 for flat grids (impenetrable flat bottom → w=0).
@inline _acoustic_w_bottom(i, j, grid, u, v, ::Nothing) = zero(grid)
...
@inline _acoustic_slope_div(i, j, k, grid, u, v, w_bottom, ::Nothing) = zero(grid)
...
- `π′_forcing`: Pre-allocated storage for explicit π' forcing during substeps
"""
struct AcousticSubstepper{N, FT, CF, AV, ST, TS}
    ...
    π′_forcing :: CF                           # Explicit π' forcing during substeps
end
...
# Flat-grid fallback: no reference state, πᵣ = 0 everywhere.
...
@kernel function _convert_slow_tendencies!(Gˢu, Gˢv, Gˢw, Gˢπ,
    ...
    @inbounds begin
        ...
        #
        # We use centered differences for ∇π. On terrain grids πᵣ varies
        # horizontally (different columns have different physical heights at
        # the same ζ index), so horizontal derivatives must use full π = πᵣ + π'.
        # On flat grids πᵣ is constant in x/y so πᵣ terms cancel; using full π
        # is correct for both cases.
        Δx = Δxᶠᶜᶜ(i, j, k, grid)
        Δy = Δyᶜᶠᶜ(i, j, k, grid)
        Δzᶜ = Δzᶜᶜᶜ(i, j, k, grid)

        # u · (∂π/∂x)_ζ: centered average of u to cell center × centered full π gradient
        uᶜ = (u[i, j, k] + u[i + 1, j, k]) / 2
        π_xp = πᵣ[i + 1, j, k] + π′[i + 1, j, k]
        π_xm = πᵣ[i - 1, j, k] + π′[i - 1, j, k]
        ∂π_∂x = (π_xp - π_xm) / (2 * Δx)

        # v · (∂π/∂y)_ζ: (zero for Flat y; centered for Periodic/Bounded)
        vᶜ = (v[i, j, k] + v[i, j + 1, k]) / 2
        π_yp = πᵣ[i, j + 1, k] + π′[i, j + 1, k]
        π_ym = πᵣ[i, j - 1, k] + π′[i, j - 1, k]
        ∂π_∂y = (π_yp - π_ym) / (2 * Δy)
...
@kernel function _compute_π′_forcing!(π′_forcing, grid, Δτ, ω̄,
    ...
    @inbounds begin
        ...
        slope_div = _acoustic_slope_div(i, j, k, grid, u, v, w_bottom, terrain_metrics)
...
function acoustic_rk3_substep_loop!(model, substepper, Δt, β_stage, U⁰)
    ...
    π′_forcing = substepper.π′_forcing
