...
``∂ₜ(ρθ)``), evaluated over the specified zone once per outer time step.
Sources are evaluated on the device, so they must capture only isbits values
(the standard boundary-condition-function restriction). A source may be
`nothing`, in which case the corresponding tendency field stays zero — a
frozen hold for that variable — unless filled in place through
...
BoundaryTendencyMarch(ρu_tendency=nothing, ρv_tendency=nothing, ρᵈ_tendency=nothing, ρθ_tendency=nothing)
```
"""
struct BoundaryTendencyMarch{U, V, R, X}
    ρu_tendency :: U
    ρv_tendency :: V
    ρᵈ_tendency :: R
    ρθ_tendency :: X
...
BoundaryTendencyMarch(; ρu_tendency = nothing,
                        ...
                        ρᵈ_tendency = nothing,
                        ρθ_tendency = nothing) =
    BoundaryTendencyMarch(ρu_tendency, ρv_tendency, ρᵈ_tendency, ρθ_tendency)
...
@kernel function _fill_boundary_tendencies!(∂ₜρu, ∂ₜρv, ∂ₜρᵈ, ∂ₜρθ, grid, scheme, open_sides, t)
...
`(ρu = ..., ρv = ..., ρᵈ = ..., ρθ = ...)` for a model whose momentum boundary
conditions carry a [`BoundaryTendencyMarch`](@ref) scheme. The fields hold
``∂ₜ(ρu)``, ``∂ₜ(ρv)``, ``∂ₜρᵈ``, ``∂ₜ(ρθ)`` over the specified zone (zero
elsewhere) and may be filled in place by a driver each outer time step as an
alternative to callable tendency sources, e.g. when the boundary data comes
from interpolated files that cannot be evaluated on the device. When callable
sources ARE provided, the fields are refreshed from them once per outer step
and external fills are overwritten.
"""
