...
@inline function compute_boussinesq_adjustment_temperature(𝒰₀::PotentialTemperatureState{FT}, thermo) where FT
    ...
    qᵛ⁺₁ = equilibrium_saturation_specific_humidity(T₁, 𝒰₁, thermo)
    qˡ₁ = qᵗ - qᵛ⁺₁
...
@inline function equilibrium_saturation_specific_humidity(T, 𝒰, thermo)
...
@inline function liquid_mass_fraction(i, j, k, grid, mb::MoistAirBuoyancy, T, θ, qᵗ)
    ...
    qᵛ⁺ = equilibrium_saturation_specific_humidity(Tᵢ, 𝒰, mb.thermodynamics)
    return max(0, qᵗᵢ - qᵛ⁺)
end
