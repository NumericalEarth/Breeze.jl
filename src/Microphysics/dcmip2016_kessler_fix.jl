...
@inline function get_tetens_parameters(svp::TetensFormula, FT)
    return svp.liquid_coefficient, svp.liquid_temperature_offset
end

@inline function get_tetens_parameters(::ClausiusClapeyron, FT)
    return FT(17.27), FT(35.85)
end

@inline function saturation_adjustment_coefficient(T_DCMIP2016, constants)
    FT = eltype(constants)
    a, _ = get_tetens_parameters(constants.saturation_vapor_pressure, FT)
...
@kernel function _microphysical_update!(microphysics, grid, Nz, Δt,
    ...
    _, δT = get_tetens_parameters(constants.saturation_vapor_pressure, FT)
