...
#
# where w(D_ice) = m_ice(D_ice) for mass collection, 1 for number, D_ice^6 for sixth moment.
...
# Per-integral weighting: m_ice for mass collection, 1 for number, D^6 for sixth moment
...
@inline _ice_rain_weight(::IceRainSixthMomentCollection, D, state, thresholds) = D^6
