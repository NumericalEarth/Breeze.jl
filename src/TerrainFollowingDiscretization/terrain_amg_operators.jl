#####
##### Terrain-aware horizontal derivatives on TFVDRG grids
#####
...
##### onto `TFVDRG`. The implementations are verbatim copies of the AMG bodies in
...
@inline Oceananigans.Operators.∂x_zᶠᶜᶜ(i, j, k, grid::TFVDRG) = δxᶠᶜᶜ(i, j, k, grid, znode, _C(), _C(), _C()) * Δx⁻¹ᶠᶜᶜ(i, j, k, grid)
@inline Oceananigans.Operators.∂x_zᶜᶜᶜ(i, j, k, grid::TFVDRG) = δxᶜᶜᶜ(i, j, k, grid, znode, _F(), _C(), _C()) * Δx⁻¹ᶜᶜᶜ(i, j, k, grid)
@inline Oceananigans.Operators.∂x_zᶠᶜᶠ(i, j, k, grid::TFVDRG) = δxᶠᶜᶠ(i, j, k, grid, znode, _C(), _C(), _F()) * Δx⁻¹ᶠᶜᶠ(i, j, k, grid)
@inline Oceananigans.Operators.∂x_zᶜᶠᶜ(i, j, k, grid::TFVDRG) = δxᶜᶠᶜ(i, j, k, grid, znode, _F(), _F(), _C()) * Δx⁻¹ᶜᶠᶜ(i, j, k, grid)
@inline Oceananigans.Operators.∂x_zᶠᶠᶜ(i, j, k, grid::TFVDRG) = δxᶠᶠᶜ(i, j, k, grid, znode, _C(), _F(), _C()) * Δx⁻¹ᶠᶠᶜ(i, j, k, grid)
@inline Oceananigans.Operators.∂x_zᶜᶜᶠ(i, j, k, grid::TFVDRG) = δxᶜᶜᶠ(i, j, k, grid, znode, _F(), _C(), _F()) * Δx⁻¹ᶜᶜᶠ(i, j, k, grid)

@inline Oceananigans.Operators.∂y_zᶜᶠᶜ(i, j, k, grid::TFVDRG) = δyᶜᶠᶜ(i, j, k, grid, znode, _C(), _C(), _C()) * Δy⁻¹ᶜᶠᶜ(i, j, k, grid)
@inline Oceananigans.Operators.∂y_zᶜᶜᶜ(i, j, k, grid::TFVDRG) = δyᶜᶜᶜ(i, j, k, grid, znode, _C(), _F(), _C()) * Δy⁻¹ᶜᶜᶜ(i, j, k, grid)
@inline Oceananigans.Operators.∂y_zᶜᶠᶠ(i, j, k, grid::TFVDRG) = δyᶜᶠᶠ(i, j, k, grid, znode, _C(), _C(), _F()) * Δy⁻¹ᶜᶠᶠ(i, j, k, grid)
@inline Oceananigans.Operators.∂y_zᶠᶜᶜ(i, j, k, grid::TFVDRG) = δyᶠᶜᶜ(i, j, k, grid, znode, _F(), _F(), _C()) * Δy⁻¹ᶠᶜᶜ(i, j, k, grid)
@inline Oceananigans.Operators.∂y_zᶠᶠᶜ(i, j, k, grid::TFVDRG) = δyᶠᶠᶜ(i, j, k, grid, znode, _F(), _C(), _C()) * Δy⁻¹ᶠᶠᶜ(i, j, k, grid)
@inline Oceananigans.Operators.∂y_zᶜᶜᶠ(i, j, k, grid::TFVDRG) = δyᶜᶜᶠ(i, j, k, grid, znode, _C(), _F(), _F()) * Δy⁻¹ᶜᶜᶠ(i, j, k, grid)

#####
##### Number-argument disambiguation (derivative of a constant is zero)
#####

@inline Oceananigans.Operators.∂xᶠᶜᶜ(i, j, k, grid::TFVDRG, c::Number) = zero(grid)
@inline Oceananigans.Operators.∂xᶜᶜᶜ(i, j, k, grid::TFVDRG, c::Number) = zero(grid)
@inline Oceananigans.Operators.∂xᶠᶜᶠ(i, j, k, grid::TFVDRG, c::Number) = zero(grid)
@inline Oceananigans.Operators.∂xᶜᶠᶜ(i, j, k, grid::TFVDRG, c::Number) = zero(grid)
@inline Oceananigans.Operators.∂xᶠᶠᶜ(i, j, k, grid::TFVDRG, c::Number) = zero(grid)

@inline Oceananigans.Operators.∂yᶜᶠᶜ(i, j, k, grid::TFVDRG, c::Number) = zero(grid)
@inline Oceananigans.Operators.∂yᶜᶜᶜ(i, j, k, grid::TFVDRG, c::Number) = zero(grid)
@inline Oceananigans.Operators.∂yᶜᶠᶠ(i, j, k, grid::TFVDRG, c::Number) = zero(grid)
@inline Oceananigans.Operators.∂yᶠᶜᶜ(i, j, k, grid::TFVDRG, c::Number) = zero(grid)
...
@inline function Oceananigans.Operators.∂xᶠᶜᶜ(i, j, k, grid::TFVDRG, f::Function, args...)
...
@inline function Oceananigans.Operators.∂xᶜᶜᶜ(i, j, k, grid::TFVDRG, ϕ)
...
@inline function Oceananigans.Operators.∂xᶜᶜᶜ(i, j, k, grid::TFVDRG, f::Function, args...)
...
@inline function Oceananigans.Operators.∂xᶠᶜᶠ(i, j, k, grid::TFVDRG, ϕ)
...
@inline function Oceananigans.Operators.∂xᶠᶜᶠ(i, j, k, grid::TFVDRG, f::Function, args...)
...
@inline function Oceananigans.Operators.∂xᶜᶠᶜ(i, j, k, grid::TFVDRG, ϕ)
...
@inline function Oceananigans.Operators.∂xᶜᶠᶜ(i, j, k, grid::TFVDRG, f::Function, args...)
...
@inline function Oceananigans.Operators.∂xᶠᶠᶜ(i, j, k, grid::TFVDRG, ϕ)
...
@inline function Oceananigans.Operators.∂xᶠᶠᶜ(i, j, k, grid::TFVDRG, f::Function, args...)
...
@inline function Oceananigans.Operators.∂yᶜᶠᶜ(i, j, k, grid::TFVDRG, f::Function, args...)
...
@inline function Oceananigans.Operators.∂yᶜᶜᶜ(i, j, k, grid::TFVDRG, ϕ)
...
@inline function Oceananigans.Operators.∂yᶜᶜᶜ(i, j, k, grid::TFVDRG, f::Function, args...)
...
@inline function Oceananigans.Operators.∂yᶜᶠᶠ(i, j, k, grid::TFVDRG, ϕ)
...
@inline function Oceananigans.Operators.∂yᶜᶠᶠ(i, j, k, grid::TFVDRG, f::Function, args...)
...
@inline function Oceananigans.Operators.∂yᶠᶜᶜ(i, j, k, grid::TFVDRG, ϕ)
...
@inline function Oceananigans.Operators.∂yᶠᶜᶜ(i, j, k, grid::TFVDRG, f::Function, args...)
...
@inline function Oceananigans.Operators.∂yᶠᶠᶜ(i, j, k, grid::TFVDRG, ϕ)
...
@inline function Oceananigans.Operators.∂yᶠᶠᶜ(i, j, k, grid::TFVDRG, f::Function, args...)
