...
using Oceananigans.Advection: vertical_scheme, implicit_vertical_velocityᶜᶜᶠ
...
"""
# First-order upwind flux of the stage-entry thermodynamic state carried by the
# CFL-withheld velocity wⁱ = (1 - s) w, density-weighted like the implicit Center-field
# coefficients so base + perturbation sum to the full-field operator.
@inline function residual_base_flux(i, j, k, grid, scheme, td, W, ρθ, ρᵈ)
...
@kernel function _residual_base_tendency!(Gρθ, grid, scheme, td, W, ρθ, ρᵈ)
    i, j, k = @index(Global, NTuple)
    @inbounds Gρθ[i, j, k] -= V⁻¹ᶜᶜᶜ(i, j, k, grid) *
        δzᵃᵃᶜ(i, j, k, grid, residual_base_flux, scheme, td, W, ρθ, ρᵈ)
end

"""
$(TYPEDSIGNATURES)

Fold the CFL-withheld transport of the stage-entry thermodynamic state into the slow
...
add_residual_base_tendency!(model, advection) =
    needs_implicit_solver(advection) ? residual_base_tendency!(model, advection) : nothing

function residual_base_tendency!(model, advection)
    grid = model.grid
...
function implicit_substep!(model, implicit_solver, Δt_stage)
    ...
    implicit_step!(prognostic[θ_name],
                   ...
                   fields(model),
                   Δt_stage,
                   implicit_step_scheme(θ_step_advection, diffusion_density),
