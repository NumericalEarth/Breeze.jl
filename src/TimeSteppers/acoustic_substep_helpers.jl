...
function compute_slow_scalar_tendencies!(model)
    ...
    # time-averaged transport velocity into the θ path (see the transport-velocity
    # note above); the velocities argument only parameterizes the microphysical state.
    # TODO: fuse this source into compute_thermodynamic_tendency!'s kernel (which
    # already builds q, 𝒰, Π, cᵖᵐ per cell); beware the anelastic core, where
    # the source already arrives via compute_microphysical_tendencies! — naive
    # fusion would double-count there.
