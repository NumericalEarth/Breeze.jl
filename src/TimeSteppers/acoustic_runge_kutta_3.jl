...
function acoustic_rk3_substep!(model::AtmosphereModel, Δt, β)
    ...
    # before the acoustic solves because it ends by moving the condensate content of the mass
    # the tracers' solves sedimented, which must then take the thermodynamic variable's own
    # implicit transport and diffusion (see `implicit_sedimentation_step!`).
