...
quantity — see [`total_air_density`](@ref) — used wherever total mass enters the physics: the
...
the two densities coincide (`total_air_density === dynamics_density`).
"""
function dynamics_density end

"""
    total_air_density(dynamics)
...
"""
total_air_density(dynamics) = dynamics_density(dynamics)
