...
A branch is a callable over `(i, j, k, grid, q, N², ℓᵗ)` through `length_scaleᶜᶜᶠ` and
...
Unlike MYNN's ``L_S`` this branch carries no stability correction; see [`BlendedMixingLength`](@ref)
for what that omits.
...
BuoyancyLengthScale(Cᵇ, N²ᵐⁱⁿ) = BuoyancyLengthScale(promote(Cᵇ, N²ᵐⁱⁿ)...)
...
hard realizability clip ``ℓ/q ≤ 1/N``, and ``N²`` enters through a smooth positive part. Two
further pieces of MYNN are simply not implemented yet — the stability correction on the surface
branch (their Eq. 53) and the convective enhancement of ``ℓᵇ`` (their Eq. 55) — so a
`BlendedMixingLength` of `GeometricLengthScale`, `TurbulenceLengthScale` and `BuoyancyLengthScale`
under `HarmonicBlend` is MYNN's *structure* rather than MYNN.
...
@inline length_scaleᶜᶜᶠ(i, j, k, grid, ℓ::GeometricLengthScale, q, N², ℓᵗ) =
    ℓ.κ * (height_above_bottomᶜᶜᶠ(i, j, k, grid) + ℓ.ℓʳ)

@inline length_scaleᶜᶜᶜ(i, j, k, grid, ℓ::GeometricLengthScale, q, N², ℓᵗ) =
    ℓ.κ * (height_above_bottomᶜᶜᶜ(i, j, k, grid) + ℓ.ℓʳ)

@inline length_scaleᶜᶜᶠ(i, j, k, grid, ::TurbulenceLengthScale, q, N², ℓᵗ) = @inbounds ℓᵗ[i, j, 1]
@inline length_scaleᶜᶜᶜ(i, j, k, grid, ::TurbulenceLengthScale, q, N², ℓᵗ) = @inbounds ℓᵗ[i, j, 1]

@inline function length_scaleᶜᶜᶠ(i, j, k, grid, ℓ::BuoyancyLengthScale, q, N², ℓᵗ)
    N²⁺ = smooth_positive(N², ℓ.N²ᵐⁱⁿ)
    return ℓ.Cᵇ * q / sqrt(N²⁺)
end

## `q` and `N²` are supplied at the caller's location, so the buoyancy branch needs no separate
## centered form.
@inline length_scaleᶜᶜᶜ(i, j, k, grid, ℓ::BuoyancyLengthScale, q, N², ℓᵗ) =
    length_scaleᶜᶜᶠ(i, j, k, grid, ℓ, q, N², ℓᵗ)
...
"""
@inline function mixing_lengthᶜᶜᶠ(i, j, k, grid, ml::BlendedMixingLength, q, N², ℓᵗ)
    ℓs = map(branch -> length_scaleᶜᶜᶠ(i, j, k, grid, branch, q, N², ℓᵗ), ml.branches)
    return ml.blend(ℓs)
end
...
"""
@inline function mixing_lengthᶜᶜᶜ(i, j, k, grid, ml::BlendedMixingLength, q, N², ℓᵗ)
    ℓs = map(branch -> length_scaleᶜᶜᶜ(i, j, k, grid, branch, q, N², ℓᵗ), ml.branches)
    return ml.blend(ℓs)
end
...
The von Kármán constant carried by the `GeometricLengthScale` branch, or `nothing` if there is
none.
