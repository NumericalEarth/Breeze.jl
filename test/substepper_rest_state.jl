...
#####
##### These tests encode the rest-state contracts for the acoustic
##### substepper: exact discrete hydrostatic balance, zero slow vertical
##### momentum seed at rest, and bounded rest-atmosphere drift.
#####
...
@testset "T1 — Reference-state hydrostatic balance" begin
    ...
    # Threshold: a few hundred ulp on a
...
##### inherits a non-physical pressure-imbalance seed.
#####
...
@testset "T2 — Pressure consistency between EoS and reference paths" begin
    ...
    # Contract: Δp ≤ 100 ulp of p, Δρ exactly zero.
...
##### working baseline is caught immediately.
#####
...
@testset "T4 — Rest-atmosphere drift Δt sweep" begin
    # Contract: a rest atmosphere on the production configuration
...
@testset "T6 — Bounded-y vs Periodic-y rest equivalence" begin
    ...
    # Bound is generous since both cases individually currently leak a
    # tiny ~1e-13 m/s drift on Float64;
