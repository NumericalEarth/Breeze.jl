...
for arch in arches
    ...
    @testset "AcousticSSPRungeKutta3 construction [$(arch), $(FT)]" for FT in as_test_float_types(arch)
        Oceananigans.defaults.FloatType = FT
        grid = RectilinearGrid(arch; size=(4, 4, 8), x=(0, 100), y=(0, 100), z=(0, 1000))
    ...
    @testset "SSP-RK3 model runs without NaN [$(arch), $(FT)]" for FT in as_test_float_types(arch)
        Oceananigans.defaults.FloatType = FT
    ...
    @testset "IGW stability: SSP-RK3 (Δt=12, Ns=8) [$(arch), $(FT)]" for FT in as_test_float_types(arch)
        Oceananigans.defaults.FloatType = FT
