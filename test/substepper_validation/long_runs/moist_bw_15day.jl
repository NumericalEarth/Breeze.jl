#####
##### F1 — Moist baroclinic wave, 15-day end-to-end validation
#####
##### DCMIP-2016 Test 1-1. Verbatim parameters from
##### `Breeze/examples/moist_baroclinic_wave.jl` — isothermal-T₀=250 K
##### reference state, 1° lat-lon (Nλ=360, Nφ=160, Nz=64),
##### latitude=(-80, 80), Lz=30 km, Δt=20 s,
##### NonEquilibriumCloudFormation(τ_relax=200 s).
#####
##### Tracks: max|w|, max|u|, max|q^cl|, surface pressure minimum,
##### conservation of mass + ρθ, wall-clock per outer step.
##### Saves snapshots every 6 hours.
#####
##### Pass criteria (SUBSTEPPER_TEST_PLAN.md F1):
##### (a) Run completes 15 days without NaN.
##### (b) max|w|(t) ≤ 5 m/s for all t.
##### (c) Cyclone deepens to surface pressure < 970 hPa by day 9.
##### (d) Mass drift ≤ 1e-8 over the full 15 days.
#####
...
const RUN_LABEL = "moist_bw_15day"
...
function diag_cb(sim)
    ...
    @info @sprintf("[moist] iter=%6d  t=%6.3fd  Δt=%.0fs  max|u|=%.2f  max|w|=%.3e  max|qcl|=%.2e  p_surf_min=%.0f Pa  ΔM=%.1e  ΔH=%.1e  wall=%.0fs",
                   iteration(sim), time(sim)/86400, sim.Δt, umax, wmax, qcl_max, psurf_min,
                   (M-M0)/M0, (H-H0)/H0, diag_wall[end])
    flush(stdout); flush(stderr)

    if isnan(wmax)
        @error "[moist] NaN in w at iter $(iteration(sim))"
        return false
    end
    return nothing
end
...
# Periodic state output every 6 hours
...
@info "Starting moist-BW 15-day run, Δt=$(Δt)s, $(stop_time/86400) days, $(Int(stop_time/Δt)) outer steps"
wall_start[] = time_ns()

try
    run!(simulation)
    @info "[moist] RUN COMPLETED"
catch e
    @error "[moist] RUN FAILED" e
end
...
@info "Moist-BW 15-day run complete. Diagnostics: $(STEM)_diagnostics.jld2"
