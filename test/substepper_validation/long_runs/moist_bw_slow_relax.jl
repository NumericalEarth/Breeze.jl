#####
##### Moist BW with τ_relax=2000s (10× slower condensation) — diagnostic.
#####
##### Same as moist_bw_no_surface_fluxes.jl but with τ_relax=2000s instead
##### of 200s. Slower relaxation → smaller condensation amplitude per
##### outer step. If the failure is "per-step condensation magnitude",
##### this should fail later (or pass 5 days). If the failure is
##### structural in microphysics-dynamics coupling regardless of rate,
##### this should fail at a similar time.
#####
...
const RUN_LABEL = "moist_bw_slow_relax"
...
# THE KEY DIFFERENCE: τ_relax=2000s instead of 200s (10× slower)
τ_relax = 2000.0
...
function diag_cb(sim)
    ...
    @info @sprintf("[slow-τ] iter=%6d t=%6.3fd Δt=%5.1fs max|u|=%.2f max|w|=%.3e max(ρqcl)=%.2e p_surf=%.0f wall=%.0fs",
                   iteration(sim), time(sim)/86400, sim.Δt, um, wm, qm, pm, diag_wall[end])
    flush(stdout)
end
add_callback!(simulation, diag_cb, IterationInterval(500))

@info "Starting slow-τ moist BW: τ_relax=$(τ_relax)s, Δt=$(Δt)s, $(stop_time/86400) days"
wall0[] = time_ns()
try
    run!(simulation)
    @info "[slow-τ] RUN COMPLETED"
catch e
    @error "[slow-τ] RUN FAILED" e
end
...
jldsave(STEM*"_diagnostics.jld2";
        ...
        wall=diag_wall, M0=M0, tau_relax=τ_relax)
