...
#
# Multi-rank MPI tests launched via mpiexec -n 4.
...
function make_simulation(model, dynamics_type; stop_iteration=100)
    if dynamics_type === :Compressible
        simulation = Simulation(model; Δt=0.05seconds, stop_iteration, verbose=false)
    else
        simulation = Simulation(model; Δt=1e-3seconds, stop_iteration, verbose=false)
...
function run_serial_simulation(; dynamics_type=:Anelastic, microphysics=nothing,
                                ...
                                stop_iteration=100, output_filename=nothing, output_fields=nothing)
    ...
    simulation = make_simulation(model, dynamics_type; stop_iteration)
    ...
    for (config_name, dynamics_type, microphysics) in configurations
        for (part_name, partition_str) in partitions
            ...
            if dynamics_type == ":Compressible"
                ...
                """
                simulation_block = """
                simulation = Simulation(model; Δt=0.05seconds, stop_iteration=$stop_iteration, verbose=false)
                """
            else
                dynamics_block = """
                reference_state = ReferenceState(grid, constants)
                ...
                """
                simulation_block = """
                simulation = Simulation(model; Δt=1e-3seconds, stop_iteration=$stop_iteration, verbose=false)
                simulation.callbacks[:wizard] = Callback(TimeStepWizard(cfl=0.5), IterationInterval(1))
                """
            end
