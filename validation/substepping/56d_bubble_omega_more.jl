#####
##### Test more ω values including 0.5 (strict centered CN).
#####
...
for ω in (0.50, 0.51, 0.52, 0.53, 0.54)
    label = "ω=$(ω)"
    @info "=== $label ==="
...
# Also try larger Ns to see if it helps
for Ns in (24, 48)
    label = "Ns=$Ns ω=0.55"
    @info "=== $label ==="
    r = run_one(label, grid -> build_substepped(grid; Ns, ω = 0.55); Δt = Δt_subst)
    mark = r.has_nan ? "NaN" : "✓"
    ratio = isnan(r.wmax_final) ? NaN : r.wmax_final / expl.wmax_final
    @info @sprintf("  %s wmax_final=%.4e ratio=%.3f", mark, r.wmax_final, ratio)
end
