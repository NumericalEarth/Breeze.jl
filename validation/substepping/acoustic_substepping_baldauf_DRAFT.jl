...
#####
##### AcousticSubstepDistribution interface (unchanged).
#####
...
#####
##### AcousticDampingStrategy interface and concrete strategies.
##### Damping is essential for stability of the substep loop (Baldauf §4); it
##### removes the spurious 2Δ acoustic mode without harming gravity-wave or
##### advective modes.
#####
...
Disable acoustic divergence damping. Use only for diagnostics; the substepper
is generally not stable without some damping at moderate-to-large CFL.
"""
struct NoDivergenceDamping <: AcousticDampingStrategy end

"""
    ThermodynamicDivergenceDamping(; coefficient = 0.1, length_scale = nothing)

Klemp–Skamarock–Ha 2018 thermodynamic divergence damping. Adds a Laplacian-like
correction to ρu, ρv proportional to the discrete `ρθ′` tendency (used as a
divergence proxy that preserves gravity-wave frequencies).
"""
...
function ThermodynamicDivergenceDamping(; coefficient = 0.1, length_scale = nothing)
    if length_scale === nothing
        ...
        return ThermodynamicDivergenceDamping{FT, Nothing}(convert(FT, coefficient), nothing)
    else
        FT = promote_type(typeof(coefficient), typeof(length_scale))
        return ThermodynamicDivergenceDamping{FT, FT}(convert(FT, coefficient), convert(FT, length_scale))
    end
end

"""
    PressureProjectionDamping(; coefficient = 0.1)

Forward pressure-projection damping (ERF/CM1/WRF style). Each substep, the PGF
source η is replaced by η + β·(η_curr − η_prev), advancing the pressure
gradient slightly into the future to damp the acoustic mode.
"""
...
@inline _convert_damping(::Type{FT}, d::PressureProjectionDamping) where FT =
    PressureProjectionDamping{FT, typeof(d.η_for_pgf)}(convert(FT, d.coefficient), d.η_for_pgf)
...
function materialize_damping(grid, d::PressureProjectionDamping{FT}) where FT
    η_for_pgf = CenterField(grid)
    return PressureProjectionDamping{FT, typeof(η_for_pgf)}(d.coefficient, η_for_pgf)
end

# Backwards-compat alias: ConservativeProjectionDamping ≡ PressureProjectionDamping (linear).
const ConservativeProjectionDamping = PressureProjectionDamping
...
@kernel function _horizontal_forward!(ρu, ρv, grid, Δτ, η_for_pgf, Πᵣ, Gρu, Gρv, Rᵈ)
    i, j, k = @index(Global, NTuple)
    @inbounds begin
        ...
        ∂x_p′ = Rᵈ * Πᶠᶜᶜ * δxᶠᵃᵃ(i, j, k, grid, η_for_pgf) / max(grid.Δxᶠᵃᵃ[i], one(eltype(grid)))
        ∂y_p′ = Rᵈ * Πᶜᶠᶜ * δyᵃᶠᵃ(i, j, k, grid, η_for_pgf) / max(grid.Δyᵃᶠᵃ[j], one(eltype(grid)))
...
@kernel function _build_σ_η_predictors!(σ_pred, η_pred,
    ...
    @inbounds begin
        ...
        σ_pred[i, j, k] = σ[i, j, k] + Δτ * (Gρ[i, j, k]  - div_M_h - (1 - ω) * d_ρw_dz)
        η_pred[i, j, k] = η[i, j, k] + Δτ * (Gρθ[i, j, k] - div_θM_h - (1 - ω) * d_θρw_dz)
    end
end
...
@inline function get_coefficient(i, j, k, grid, ::AcousticTridiagLower, p, ::ZDirection,
                                 Πᵣ, ρθᵣ, ρᵣ, Δτ, Rᵈ, g, ω)
    # Lower at solver row k_s couples to row k_s-1, i.e. face k+1 with face k.
    k_face = k + 1
    ...
    # PGF coupling of ρw_new(k_face-1) into η at center k_face-1 → into face k_face
    pgf_coupling  = -pgf_factor * θᵣ_below_face_below / Δzᶜ_below
    # Buoyancy coupling of ρw_new(k_face-1) into σ at center k_face-1 → face k_face
...
@inline function get_coefficient(i, j, k, grid, ::AcousticTridiagDiagonal, p, ::ZDirection,
                                 Πᵣ, ρθᵣ, ρᵣ, Δτ, Rᵈ, g, ω)
    # Boundary row at k=1 ↔ ρw=0: trivial b=1 with f=0.
    k == 1 && return one(Δτ)
    k_face = k
    ...
    pgf_factor = _tridiag_pgf_coeff(i, j, k_face, grid, Πᵣ, ρθᵣ, ρᵣ, Δτ, ω, Rᵈ)
    # PGF coupling of ρw_new(k_face) into η at centers k_face and k_face-1.
    pgf_coupling  = pgf_factor * θᵣ_face * (1 / Δzᶜ_above + 1 / Δzᶜ_below)
    # Buoyancy coupling: ρw_new(k_face) divergence in mass at centers above and below face.
...
function apply_pgf_filter!(damping::PressureProjectionDamping, substepper, model)
    ...
    launch!(arch, grid, :xyz, _pressure_projection_filter!,
            damping.η_for_pgf, substepper.η, substepper.η_prev, β)
    fill_halo_regions!(damping.η_for_pgf)
    return nothing
end

@kernel function _pressure_projection_filter!(η_for_pgf, η, η_prev, β)
    i, j, k = @index(Global, NTuple)
    @inbounds η_for_pgf[i, j, k] = η[i, j, k] + β * (η[i, j, k] - η_prev[i, j, k])
end
...
@inline pgf_source_field(damping::PressureProjectionDamping, substepper) = damping.η_for_pgf
...
function acoustic_rk3_substep_loop!(model, substepper, Δt, β_stage, U⁰)
    ...
    for substep in 1:Nτ
        ...
        # Optional pressure-projection damping fills `damping.η_for_pgf`.
        apply_pgf_filter!(substepper.damping, substepper, model)
        η_for_pgf = pgf_source_field(substepper.damping, substepper)

        # 1. Forward step for ρu, ρv.
        launch!(arch, grid, :xyz, _horizontal_forward!,
                substepper.ρu, substepper.ρv, grid, Δτ_FT,
                η_for_pgf, substepper.reference_exner_function,
                Gρu, Gρv, FT(Rᵈ))
        fill_halo_regions!(substepper.ρu)
        fill_halo_regions!(substepper.ρv)
