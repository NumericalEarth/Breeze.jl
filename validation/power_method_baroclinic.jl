...
# integrating a balanced jet forward three days, then rescaling perturbations
# to a reference amplitude. After enough iterations the perturbation locks onto
# the most unstable normal mode (wavenumber 9). Their σ ≈ 0.46 day⁻¹ at 1°
# resolution is for the [JablonowskiWilliamson2006](@citet) jet, which is *not*
# the jet integrated here: this script uses the hotter, more strongly-sheared
# DCMIP2016 jet [UllrichEtAl2016](@citet), whose most-unstable wavenumber-9
# growth rate is substantially faster (σ ≈ 0.75 day⁻¹). We reproduce the Park
# power-method *procedure*, not their JW2006 growth rate.
#
# The algorithm:
#
...
# ## Grid
#
# A 1° latitude-longitude grid spanning 75° S to 75° N, up to 30 km with
# 64 vertical levels exponentially stretched toward the surface — the same
# grid as `baroclinic_wave.jl`.
...
## Base state is the *balanced* jet only — the localized DCMIP2016 §3.3
## trigger is deliberately omitted so the power method grows its own seeded
## wavenumber-9 perturbation about the steady state (matches the example's
## balanced jet; the trigger is a nonlinear-example device, not a base state).
...
for n in 1:max_iterations
    ...
    ## The broadcast covers the full parent array (halos included, since `background`
    ## was snapshotted from parent()), so halos are rescaled consistently and the
    ## `update_state!` below refills them from the interior before the next run.
    ...
    ## Guard the denominator so a near-zero σ (transient decay before the mode
    ## locks on) cannot trigger a spurious "convergence".
    ...
    ## Reset the clock for the next iteration; update_state! also refills halos.
    reset!(model.clock)
    update_state!(model, compute_tendencies=false)
end
...
## Expected DCMIP2016 wavenumber-9 rate (this jet), not Park's JW2006 σ = 0.46.
hlines!(ax, [0.75]; linestyle=:dash, color=:gray40, label="DCMIP2016 jet (≈0.75)")
hlines!(ax, [0.46]; linestyle=:dot, color=:gray70, label="Park et al. JW2006 (0.46)")
axislegend(ax; position=:rb)

save("validation/output/power_method_convergence.png", fig)
nothing #hide
...
save("validation/output/power_method_eigenmode.png", fig2)
nothing #hide

# ## Record converged growth rate + eigenmode

σ_converged = σ_history[end] * 86400

@info @sprintf("converged σ = %.4f day⁻¹ after %d iterations (max|v|=%.3e, max|δθ|=%.3e)",
               σ_converged, length(σ_history), vlim, δθlim)
