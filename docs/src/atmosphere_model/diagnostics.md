...
These diagnostics can be computed as [`KernelFunctionOperation`](@ref Oceananigans.AbstractOperations.KernelFunctionOperation)s,
which allows them to be evaluated lazily or wrapped in [`Field`](@ref Oceananigans.Fields.Field)s for storage and output.
...
- **`snake_case` names** (e.g., `temperature`, `density`): These functions may return either
  a [`Field`](@ref Oceananigans.Fields.Field) or a `KernelFunctionOperation`, depending on
