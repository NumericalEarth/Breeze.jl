...
| ``\ell^b``                          | `ℓᵇ`   |                                     | Buoyancy penetration depth, ``ℓᵇ = \sqrt{e} / N``, how far an eddy with kinetic energy ``e`` penetrates air of buoyancy frequency ``N``; infinite where ``N² ≤ 0``. Every mixing-length formulation is built from it and the wall length ``Cˢ z``: the local minimum ``\min(Cˢ z, ℓᵇ)``, its gradient-limited envelope ``\min_{z′} [ℓᵇ(z′) + Cˢ \|z - z′\|]`` (the default), or the Bougeault–Lacarrère parcel lengths |
...
| ``C^{χ-}, C^{χ0}, C^{χ+}``          | `Cᵘ⁻`, `Cᵘ⁰`, `Cᵘ⁺`, … |                    | Endpoints of the Richardson-number-dependent stability functions, ``χ ∈ \{u, c, e, D\}``: the unstable, neutral and stable value of ``S^χ``. Twelve in all, shared by `PiecewiseStabilityFunction` and `RationalStabilityFunction` |
| ``Ri⁰, Ri^\delta``                  | `Ri⁰`, `Riᵟ` |                              | Onset and width of the stable ramp of `PiecewiseStabilityFunction` |
| ``Ri^-, Ri^+``                      | `Ri⁻`, `Ri⁺` |                              | Transition scales of `RationalStabilityFunction`: the ``\|Ri\|`` at which every ``S^χ`` stands halfway between ``C^{χ0}`` and its unstable or stable limit |
| ``\mathcal{D}``                     | `𝒟`    |                                     | Signed saturation excess, ``𝒟 = qʷ - qˢ``, a mass fraction: the condensate where the air is saturated, the vapor deficit below saturation where it is not. Distinct from ``S^D``/``C^D``, the dissipation stability function |
| ``σ_\mathcal{D}``                   | `σ𝒟`   |                                     | Width of the unresolved distribution of ``𝒟`` in `ConditionalStabilityMixingLength` |
| ``C_\mathcal{D}^g, C_\mathcal{D}^0`` | `C𝒟ᵍ`, `C𝒟⁰` |                          | Gradient and background contributions to ``σ_𝒟²  = C_𝒟^g ℓ₀² (∂_z 𝒟)² + (C_𝒟^0)²`` |
| ``C^h, C^{\mathrm{cond}}``          | `Cʰ`, `Cᶜᵒⁿᵈ` |                          | Height of the trial excursion in units of ``ℓ₀``, and the strength of the conditional weakening, in ``[0, 1]``; zero recovers the wrapped mixing length |
| ``N_m^2``                           | `Nₘ²`  |                                     | Hypothetical saturated static stability used by the conditional correction; the actual ``N²`` where the air is saturated |
| ``p^-, p^+``                        | `p⁻`, `p⁺` |                                | Exponents of `RationalStabilityFunction`, one by default and fixed for calibration; the shape is ``x^p/(1+x^p)`` with ``x = \|Ri\|/Ri^\pm`` |
