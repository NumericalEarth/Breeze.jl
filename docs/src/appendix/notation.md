...
* The mixture momentum equation includes the full pressure-gradient force and the total gravitational force ``ρ g``. Because the prognostic momentum is coupling-weighted (``ρu = ρᵈ u``), these forces are multiplied by ``qᵈ = ρᵈ/ρ`` in the tendency, using `dynamics_mass_fractionᶠᶜᶜ` and its ``ᶜᶠᶜ``/``ᶜᶜᶠ`` counterparts; this factor is identically 1 on the anelastic core. Advection, Coriolis and stress-divergence terms are not multiplied by ``qᵈ``.
...
| ``qᵈ``                              | `qᵈ`   | `dynamics_mass_fractionᶠᶜᶜ(i, j, k, grid, dynamics)` | Dry-air mass fraction, ``qᵈ = ρᵈ/ρ = 1 - qᵗ``: rescales a mixture force density (pressure gradient, gravity) onto the coupling-weighted momentum ``ρu = ρᵈ u``. Also the ``qᵈ`` of ``Rᵐ = qᵈ Rᵈ + qᵛ Rᵛ`` |
