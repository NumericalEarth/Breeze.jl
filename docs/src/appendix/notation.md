...
* Because momentum is coupling-weighted, the *mixture* forces — the pressure gradient and gravity, which act on the total mass ``ρ`` — enter the momentum tendency multiplied by ``qᵈ = ρᵈ/ρ`` (`coupling_mass_fractionᶠᶜᶜ` and its ``ᶜᶠᶜ``/``ᶜᶜᶠ`` counterparts, identically 1 on the anelastic core). Advection, Coriolis and the stress divergence take no factor.
...
| ``qᵈ``                              | `qᵈ`   | `coupling_mass_fractionᶠᶜᶜ(i, j, k, grid, dynamics)` | Dry-air (coupling) mass fraction, ``qᵈ = ρᵈ/ρ = 1 - qᵗ``: converts a mixture force density (pressure gradient, gravity) into a force on the coupling-weighted momentum ``ρu = ρᵈ u``. Also the ``qᵈ`` of ``Rᵐ = qᵈ Rᵈ + qᵛ Rᵛ`` |
