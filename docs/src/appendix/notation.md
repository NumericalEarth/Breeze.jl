...
| ``∂T/∂rˡ``                          | `∂T∂rˡ` | `phase_change_temperature_slope(𝒰, constants)` | Temperature change per unit of vapor mixing ratio converted to liquid at fixed total moisture and fixed prognostic invariant (``θˡⁱ`` or ``s``); linearizes the Kessler saturation adjustment |
