...
| ``\ell^b``                          | `ℓᵇ`   |                                     | Buoyancy penetration depth, ``ℓᵇ = \sqrt{e} / N``, how far an eddy with kinetic energy ``e`` penetrates air of buoyancy frequency ``N``; infinite where ``N² ≤ 0`` and zero at the ground. The primary mixing length is its envelope over the column, ``ℓ(z) = \min_{z′} [ℓᵇ(z′) + Cˢ \|z - z′\|]``, which is ``\min(Cˢ z, ℓᵇ)`` where only the ground and the level itself bind |
