...
#####
#
# Microphysics schemes implement the gridless microphysical_state(microphysics, ρ, μ, 𝒰, w)
...
# The w argument is required for interface compatibility but not used by one-moment schemes.
@inline function AM.microphysical_state(bμp::WP1M, ρ, μ, 𝒰, w)
...
@inline function AM.microphysical_state(bμp::WPNE1M, ρ, μ, 𝒰, w)
    qᶜˡ = μ.ρqᶜˡ / ρ
    qʳ = μ.ρqʳ / ρ
    return WarmPhaseOneMomentState(qᶜˡ, qʳ)
end
...
@inline function AM.microphysical_state(bμp::MP1M, ρ, μ, 𝒰, w)
    q = 𝒰.moisture_mass_fractions
    qʳ = μ.ρqʳ / ρ
    qˢ = μ.ρqˢ / ρ
    qᶜˡ = max(zero(qʳ), q.liquid - qʳ)  # cloud liquid = total liquid - rain
...
@inline function AM.microphysical_state(bμp::MPNE1M, ρ, μ, 𝒰, w)
    qᶜˡ = μ.ρqᶜˡ / ρ
    qᶜⁱ = μ.ρqᶜⁱ / ρ
    qʳ = μ.ρqʳ / ρ
    qˢ = μ.ρqˢ / ρ
