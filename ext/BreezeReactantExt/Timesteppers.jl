...
using Breeze.TimeSteppers: SSPRungeKutta, CompressibleAcousticModel

function OceananigansTimeSteppers.first_time_step!(model::AtmosphereModel{<:Any, <:Any, <:ReactantState, <:SSPRungeKutta}, Δt)
    initialize!(model)
    update_state!(model)
    time_step!(model, Δt)
    return nothing
