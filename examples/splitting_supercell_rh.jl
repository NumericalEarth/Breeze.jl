...
# Parameters are defined as variables for plotting, but GPU functions use literal values
# to avoid type instability from captured globals.

FT = eltype(grid)

# Parameters for documentation and plotting (not captured by GPU functions)
...
zₛ = 5000       # m - shear layer height
uₛ = 30         # m/s - maximum shear wind speed
uᶜ = 15         # m/s - storm motion (Galilean translation speed)
nothing #hide

# Thermodynamic constants
g = constants.gravitational_acceleration    # ≈ 9.80665 m/s²
cᵖᵈ = constants.dry_air.heat_capacity       # ≈ 1004.64 J/(kg·K)
nothing #hide

# Background potential temperature profile (Equation 14 in [Klemp et al. (2015)](@cite KlempEtAl2015)):
# Uses hardcoded literal values for GPU type stability.

@inline function θ_background(z)
    # θ₀ = 300, θₜᵣ = 343, zₜᵣ = 12000, Tₜᵣ = 213, g/cᵖᵈ ≈ 0.00976
    FT = typeof(z)
    θ_troposphere = FT(300) + FT(43) * (z / FT(12000))^FT(1.25)
    θ_stratosphere = FT(343) * exp(FT(0.00976) / FT(213) * (z - FT(12000)))
    in_troposphere = z <= FT(12000)
    return ifelse(in_troposphere, θ_troposphere, θ_stratosphere)
end
nothing #hide

# Relative humidity profile (decreases with height, 25% above tropopause):

@inline function ℋ_background(z)
    # zₜᵣ = 12000
    FT = typeof(z)
    ℋ_troposphere = FT(1) - FT(0.75) * (z / FT(12000))^FT(1.25)
    ℋ_stratosphere = FT(0.25)
    in_troposphere = z <= FT(12000)
    return ifelse(in_troposphere, ℋ_troposphere, ℋ_stratosphere)
end
nothing #hide

# Zonal wind profile with linear shear below ``z_s`` and smooth transition (Equations 15-16):

@inline function u_background(z)
    # zₛ = 5000, uₛ = 30, uᶜ = 15
    FT = typeof(z)
    z_ratio = z / FT(5000)
    u_lower = FT(30) * z_ratio - FT(15)
    u_transition = (FT(-0.8) + FT(3) * z_ratio - FT(1.25) * z_ratio^2) * FT(30) - FT(15)
    u_upper = FT(15)  # uₛ - uᶜ = 30 - 15
    in_lower = z < FT(4000)   # zₛ - 1000
...
# ## Initial conditions
#
# We visualize the background atmospheric profiles (potential temperature, relative humidity,
# and wind shear) along with the warm bubble perturbation that triggers convection.

# Warm bubble parameters following Equations 17–18 in [Klemp et al. (2015)](@cite KlempEtAl2015):

Δθ = 3              # K - perturbation amplitude
rₕ = 10000          # m - bubble horizontal radius (10 km)
rᵥ = 1500           # m - bubble vertical radius
zᵦ = 1500           # m - bubble center height
xᵦ = Lx / 2         # m - bubble center x-coordinate (84 km)
yᵦ = Ly / 2         # m - bubble center y-coordinate (84 km)

# The total initial potential temperature combines the background profile with the
# cosine-squared warm bubble perturbation. Uses hardcoded literals for GPU type stability.

@inline function θᵢ(x, y, z)
    # Δθ = 3, rₕ = 10000, rᵥ = 1500, zᵦ = 1500, xᵦ = yᵦ = 84000
    FT = typeof(x)
    θ_base = θ_background(z)
    r = sqrt((x - FT(84000))^2 + (y - FT(84000))^2)
    R = sqrt((r / FT(10000))^2 + ((z - FT(1500)) / FT(1500))^2)
    θ_pert = ifelse(R < 1, FT(3) * cos(FT(π) / 2 * R)^2, FT(0))
    return θ_base + θ_pert
end

@inline uᵢ(x, y, z) = u_background(z)

# Compute profiles for plotting:
...
# Create combined figure with background profiles and warm bubble perturbation:
...
# Bottom row: warm bubble perturbation spanning all columns
axbubble = Axis(fig[2, 1:3], xlabel="x (km)", ylabel="z (km)",
                title="Warm bubble perturbation θ′ (K) at y = Ly/2")
hm = heatmap!(axbubble, x_slice ./ 1000, z_slice ./ 1000, θ′_slice,
              colormap=:thermal, colorrange=(0, Δθ))
Colorbar(fig[2, 4], hm, label="θ′ (K)")
...
# ## Initial conditions
#
# We set the initial potential temperature (with warm bubble) and wind profile first,
# then set relative humidity ``ℋ`` which internally computes ``qᵗ = ℋ × qᵛ⁺`` where
# ``qᵛ⁺`` is the saturation specific humidity at the current temperature.

@inline ℋᵢ(x, y, z) = ℋ_background(z)
set!(model, θ=θᵢ, u=uᵢ, ℋ=ℋᵢ)
