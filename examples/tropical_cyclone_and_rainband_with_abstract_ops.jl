...
Tˢ_bcs = FieldBoundaryConditions(bottom=ValueBoundaryCondition(Tˢ_data[1]))
θˢ_bcs = FieldBoundaryConditions(bottom=ValueBoundaryCondition(θˢ_data[1]))
...
axqᵗˢ = Axis(fig[1, 2], xlabel="Total Moisture (kg/kg)", ylabel="Height (m)", title="Dunion 2011 Sounding: Potential Temperature")
...
ΔT = Tˢ - T_out_K
∂z_R = 1 / (2 * ΔT * ∂z(Tˢ))
R = CumulativeIntegral(∂z_R, dims=3) |> Field

radius_of_angular_momentum_surface = CenterField(grid) #Array{Float64}(undef, Nz)
radius_of_angular_momentum_surface[1] = RMW
for k in 2:Nz
    z_k = znodes(grid, Center())[k]
    Δz_k = zspacing(i, j, k, grid, Center(), Center(), Center())
    dTdZ = (T_sounding_interp(z_k + Δz_k) - T_sounding_interp(z_k)) / Δz_k
...
for k in 1:Nz_p
    ...
    p_background = reference_state.surface_pressure * exp(- g * z_k / (Rᵈ * T_k))
