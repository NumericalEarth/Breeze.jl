...
function total_energy_density(model)
    return static_energy_density(model) |> Field
end

function kinetic_energy_density(model)
    u, v, w = model.velocities
    ρᵣ = model.formulation.reference_state.density
    qᵗ = model.specific_moisture
    return @at (Center, Center, Center) 0.5 * ρᵣ * (1 + qᵗ) * (u^2 + v^2 + w^2) |> Field
end
...
Eρ = total_energy_density(model)
Kρ = kinetic_energy_density(model)
∫Eρ = Field(Integral(Eρ))
∫Kρ = Field(Integral(Kρ))
...
function progress(sim)
    ...
    msg = @sprintf("Iter: % 4d, t: % 14s, Δt: % 14s, E: %.8e J, extrema(θ): (%.2f, %.2f) K, max|w|: %.2f m/s",
                   iteration(sim), prettytime(sim), prettytime(sim.Δt), total_E, extrema(θ)..., maximum(abs, w))
    msg *= @sprintf(", KE: %.8e J", total_K)
    @info msg
    return nothing
end
...
u, v, w = model.velocities
outputs = (; θ, w, total_energy_density=Eρ, total_energy=∫Eρ, kinetic_energy_density=Kρ, kinetic_energy=∫Kρ)
...
w_levels = -floor(w_range / 2) * 2 : 2 : floor(w_range / 2) * 2
contour!(axw, wn, levels = w_levels, color = :black, linewidth = 0.5)
...
Eρ = total_energy_density(moist_model)
Kρ = kinetic_energy_density(moist_model)
∫Eρ = Field(Integral(Eρ))
∫Kρ = Field(Integral(Kρ))
...
function progress_moist(sim)
    ...
    msg = @sprintf("Iter: % 4d, t: % 14s, Δt: % 14s, E: %.8e J, KE: %.8e J, extrema(θ): (%.2f, %.2f) K \n",
                   iteration(sim), prettytime(sim), prettytime(sim.Δt), total_E, total_K, extrema(θ)...)
...
# Total mass
ρᵣ = moist_model.formulation.reference_state.density
ρ = ρᵣ * (1 + qᵗ)
M = Integral(ρ) |> Field

moist_outputs = (; θ, w, qˡ′, kinetic_energy_density=Kρ, kinetic_energy=∫Kρ, total_energy_density=Eρ, total_energy=∫Eρ, M)
