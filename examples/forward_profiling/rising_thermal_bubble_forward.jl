...
function main()
    ...
    # Precision toggle for forward benchmarking.
    # Switch to `Float64` for an A/B comparison against the previous behavior.
    float_type = Float32
    Oceananigans.defaults.FloatType = float_type
    @info "Using floating-point precision" float_type

    # Physical parameters
    θ_background           = float_type(300.0)   # [K]
    perturbation_amplitude = float_type(2.0)     # [K]
    perturbation_radius    = float_type(1000.0)  # [m]
    bubble_center_x        = float_type(5000.0)  # [m]
    bubble_center_y        = float_type(5000.0)  # [m]
    bubble_center_z        = float_type(2000.0)  # [m]
    latitude               = float_type(45.0)    # [degrees] for f-plane Coriolis
    ...
    if !disable_benchmark
        ...
        ax_bench = Axis(fig_bench[1, 1];
                        xticks = (1:n, xlabels),
                        ylabel = "Wall time (s)",
                        title = @sprintf("Forward-only benchmark: Julia GPU vs Reactant GPU (nsteps = %d)", nsteps),
                        yscale = log10)
