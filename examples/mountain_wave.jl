...
# Use a representative mid-level reference momentum density for the BC
# This is a simple approximation that works for most cases
ρᵣ_mid = 0.7  # approximate mid-level density (kg/m³)
ρu_ref = ρᵣ_mid * Uᵢ  # reference momentum density at boundaries
open_bc = OpenBoundaryCondition(ρu_ref; scheme)
...
# Use the reference density from the ReferenceState created above
...
# We use a conservative CFL number to ensure stability with the open boundaries.

simulation = Simulation(model; Δt=0.5, stop_time=1hour)
