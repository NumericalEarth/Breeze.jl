...
# ```
#
# where ``Cᴰ, Cᵀ, Cᵛ`` are exchange coefficients, ``|ΔU| = [(u - u₀)^2 + (v - v₀)^2 + Uᵍ^2]^{1/2}``
# is "total" differential wind speed (including gustiness ``Uᵍ``), and ``θ₀, qᵛ₀`` are the surface temperature
