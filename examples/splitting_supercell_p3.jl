# # Splitting supercell with P3 microphysics
#
...
#
# For microphysics we use the P3 scheme, which includes prognostic cloud water
# rain water, and ice with autoconversion, accretion, rain evaporation, and sedimentation processes.
...
using Breeze
using Breeze: PredictedParticlePropertiesMicrophysics, TetensFormula
...
save("supercell_p3_warm_bubble.png", fig) #src
fig

# ## Model setup
#
# We use the Predicted Particle Properties (P3) microphysics scheme with high-order WENO advection.
...
conjure_time_step_wizard!(simulation, cfl=0.7, max_Δt=4.0)
...
qⁱxy_ts = FieldTimeSeries(slices_filename, "qⁱxy")
...
qⁱlim = max(maximum(qⁱxy_ts) / 4, 1e-6)
...
axqⁱ = Axis(fig[1, 4], aspect=1, xlabel="x (m)", ylabel="y (m)", title="qⁱ (kg/kg)")

n = Observable(1)
wxy_n = @lift wxy_ts[$n]
qᶜˡxy_n = @lift qᶜˡxy_ts[$n]
qʳxy_n = @lift qʳxy_ts[$n]
qⁱxy_n = @lift qⁱxy_ts[$n]
title = @lift "Splitting supercell with P3 microphysics at z ≈ 5 km, t = " * prettytime(times[$n])
...
hmqⁱ = heatmap!(axqⁱ, qⁱxy_n, colormap=:dense, colorrange=(0, qⁱlim))

Colorbar(fig[2, 1], hmw, vertical=false)
Colorbar(fig[2, 2], hmqᶜˡ, vertical=false)
Colorbar(fig[2, 3], hmqʳ, vertical=false)
Colorbar(fig[2, 4], hmqⁱ, vertical=false)

fig[0, :] = Label(fig, title, fontsize=14, tellwidth=false)

CairoMakie.record(fig, "splitting_supercell_p3_slices.mp4", 1:Nt, framerate=10) do nn
    n[] = nn
end
nothing #hide

# ![](splitting_supercell_p3_slices.mp4)
